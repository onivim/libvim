...
apply_options(win_T *wp, buf_T *buf UNUSED, dict_T *dict) {
  ...
  if (di != NULL) {
    ...
    } else if (di->di_tv.v_type == VAR_LIST && di->di_tv.vval.v_list != NULL && di->di_tv.vval.v_list->lv_len == 2) {
...
void popup_adjust_position(win_T *wp) {
  ...
  for (lnum = 1; lnum <= wp->w_buffer->b_ml.ml_line_count; ++lnum) {
    ...
    } else if (len > maxwidth && allow_adjust_left && (wp->w_popup_pos == POPPOS_TOPLEFT || wp->w_popup_pos == POPPOS_BOTLEFT)) {
...
popup_create(typval_T *argvars, typval_T *rettv, create_type_T type) {
  ...
  if (!(argvars[0].v_type == VAR_STRING && argvars[0].vval.v_string != NULL) && !(argvars[0].v_type == VAR_LIST && argvars[0].vval.v_list != NULL)) {
    emsg(_(e_listreq));
    return;
  }
...
void ex_popupclear(exarg_T *eap UNUSED) {
  close_all_popups();
}
...
#if defined(FEAT_TIMERS)
    dict_add_number(dict, "time", wp->w_popup_timer != NULL ? (long)wp->w_popup_timer->tr_interval : 0L);
#endif
  }
}
...
find_next_popup(int lowest) {
  ...
  found_wp = NULL;
  for (wp = first_popupwin; wp != NULL; wp = wp->w_next)
    if ((wp->w_popup_flags & (POPF_HANDLED | POPF_HIDDEN)) == 0 && (lowest ? wp->w_zindex < found_zindex
...
void popup_check_cursor_pos() {
  ...
  while ((wp = find_next_popup(TRUE)) != NULL)
    if (wp->w_popup_curwin != NULL && (curwin != wp->w_popup_curwin || curwin->w_cursor.lnum != wp->w_popup_lnum || curwin->w_cursor.col < wp->w_popup_mincol || curwin->w_cursor.col > wp->w_popup_maxcol)) {
