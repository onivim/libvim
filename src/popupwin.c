...
static poppos_entry_T poppos_entries[] = {{"botleft", POPPOS_BOTLEFT},
 ...
 */
static int popup_options_one(dict_T *dict, char_u *key) {
  dictitem_T *di;
  char_u *val;
  char_u *s;
  char_u *endp;
...
static void get_pos_options(win_T *wp, dict_T *dict) {
  char_u *str;
  int nr;
 ...
 */
static void set_moved_values(win_T *wp) {
 ...
 */
static void set_moved_columns(win_T *wp, int flags) {
  char_u *ptr;
 ...
 * popup window "wp".
 */
static void apply_options(win_T *wp, buf_T *buf UNUSED, dict_T *dict) {
  int nr;
  char_u *str;
  dictitem_T *di;
  int i;
  ...
  if (di != NULL) {
    if (di->di_tv.v_type != VAR_LIST)
      ...
      if (list->lv_len == 1 && wp->w_border_highlight[0] != NULL)
        for (i = 1; i < 4; ++i)
          wp->w_border_highlight[i] = vim_strsave(wp->w_border_highlight[0]);
    }
  }
  ...
  if (di != NULL) {
    ...
    } else if (di->di_tv.v_type == VAR_LIST && di->di_tv.vval.v_list != NULL &&
 ...
 */
static void add_popup_strings(buf_T *buf, list_T *l) {
  listitem_T *li;
  linenr_T lnum = 0;
  char_u *p;
 ...
 */
static void add_popup_dicts(buf_T *buf, list_T *l) {
  listitem_T *li;
  listitem_T *pli;
  linenr_T lnum = 0;
  char_u *p;
  ...
  for (li = l->lv_first; li != NULL; li = li->li_next) {
    ...
    p = dict == NULL ? NULL : dict_get_string(dict, (char_u *)"text", FALSE);
...
typedef enum { TYPE_NORMAL, TYPE_ATCURSOR } create_type_T;
 ...
 */
static win_T *find_popup_win(int id) {
 ...
 */
static void invoke_popup_callback(win_T *wp, typval_T *result) {
  typval_T rettv;
  int dummy;
  typval_T argv[3];
 ...
 */
static void popup_close_and_callback(win_T *wp, typval_T *arg) {
  int id = wp->w_id;
 ...
 * popup_close({id})
 */
void f_popup_close(typval_T *argvars, typval_T *rettv UNUSED) {
 ...
 * popup_hide({id})
 */
void f_popup_hide(typval_T *argvars, typval_T *rettv UNUSED) {
  int id = (int)tv_get_number(argvars);
  win_T *wp = find_popup_win(id);
 ...
 * popup_show({id})
 */
void f_popup_show(typval_T *argvars, typval_T *rettv UNUSED) {
  int id = (int)tv_get_number(argvars);
  win_T *wp = find_popup_win(id);
...
static void popup_free(win_T *wp) {
...
void ex_popupclear(exarg_T *eap UNUSED) { close_all_popups(); }

/*
 * popup_move({id}, {options})
 */
void f_popup_move(typval_T *argvars, typval_T *rettv UNUSED) {
  dict_T *d;
  int nr;
  int id = (int)tv_get_number(argvars);
  win_T *wp = find_popup_win(id);
...
void f_popup_getoptions(typval_T *argvars, typval_T *rettv) {
  ...
  if (rettv_dict_alloc(rettv) == OK) {
    ...
    for (i = 0; i < (int)(sizeof(poppos_entries) / sizeof(poppos_entry_T)); ++i)
      if (wp->w_popup_pos == poppos_entries[i].pp_val) {
        dict_add_string(dict, "pos", (char_u *)poppos_entries[i].pp_name);
        break;
      }
...
win_T *find_next_popup(int lowest) {
  ...
  for (wp = first_popupwin; wp != NULL; wp = wp->w_next)
    if ((wp->w_popup_flags & (POPF_HANDLED | POPF_HIDDEN)) == 0 &&
        (lowest ? wp->w_zindex < found_zindex : wp->w_zindex > found_zindex)) {
 ...
 */
static int invoke_popup_filter(win_T *wp, int c) {
  int res;
  typval_T rettv;
  int dummy;
  typval_T argv[3];
