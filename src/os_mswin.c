 ...
 */
void SaveInst(HINSTANCE hInst) { g_hinst = hInst; }
#endif
 ...
 * Return OK or FAIL.
 */
int mch_FullName(char_u *fname, char_u *buf, int len, int force UNUSED) {
  int nResult = FAIL;
  WCHAR *wname;
  WCHAR wbuf[MAX_PATH];
  char_u *cname = NULL;
...
static int wstat_symlink_aware(const WCHAR *name, stat_T *stp) {
 ...
 */
void mch_suspend(void) { suspend_shell(); }
 ...
 */
static size_t check_str_len(char_u *str) {
  SYSTEM_INFO si;
...
void __cdecl Trace(char *pszFormat, ...) {
  CHAR szBuff[2048];
  va_list args;
 ...
 */
static void GetConsoleHwnd(void) {
...
static char_u *resolve_reparse_point(char_u *fname) {
  ...
  if (!pGetVolumeInformationByHandleW(h, NULL, 0, &snfile, NULL, NULL, NULL, 0))
    goto fail;
  ...
  do {
    GetVolumeInformationW(buff, NULL, 0, &snfind, NULL, NULL, NULL, 0);
    if (snfind == snfile)
      break;
  ...
  if (!GetVolumePathNamesForVolumeNameW(buff, volnames, size, &size))
    goto fail;
 ...
 */
static char_u *resolve_shortcut(char_u *fname) {
  HRESULT hr;
  IShellLink *psl = NULL;
...
static struct quality_pair quality_pairs[] = {
...
static int CALLBACK font_enumproc(ENUMLOGFONTW *elf, NEWTEXTMETRICW *ntm UNUSED,
  ...
  *lf = elf->elfLogFont;
  ...
  if (lf->lfCharSet == ANSI_CHARSET || lf->lfCharSet == OEM_CHARSET ||
...
static int init_logfont(LOGFONTW *lf) {
  int n;
 ...
 */
static int utf16ascncmp(const WCHAR *w, const char *p, size_t n) {
  size_t i;
