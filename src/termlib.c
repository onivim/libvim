...
int tgetent(char *tbuf, /* Buffer to hold termcap entry, TBUFSZ bytes max */
...
static int getent(char *tbuf, char *term, FILE *termcap, int buflen) {
  char *tptr;
  int tlen = strlen(term);
 ...
 */
static int nextent(char *tbuf, FILE *termcap, int buflen) {
...
char *tgoto(char *cm, /* cm string, from termcap */
...
long _bauds[16] = {0,   50,   75,   110,  134,  150,  200,   300,
                   600, 1200, 1800, 2400, 4800, 9600, 19200, 19200};

int tputs(char *cp,                   /* string to print */
 ...
 */
static char *_addfmt(char *buf, char *fmt, int val) {
  sprintf(buf, fmt, val);
  while (*buf)
    buf++;
  return buf;
