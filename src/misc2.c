 ...
 */
int inc_cursor(void) { return inc(&curwin->w_cursor); }
 ...
 */
int dec_cursor(void) { return dec(&curwin->w_cursor); }
 ...
 */
void check_cursor_col(void) { check_cursor_col_win(curwin); }
...
void check_cursor_col_win(win_T *win) {
  ...
  if (len == 0)
    ...
    if ((State & INSERT) || restart_edit || (VIsual_active && *p_sel != 'o') ||
        (ve_flags & VE_ONEMORE) || virtual_active())
...
static void mem_pre_alloc_s(size_t *sizep) { *sizep += sizeof(size_t); }

static void mem_pre_alloc_l(size_t *sizep) { *sizep += sizeof(size_t); }
 ...
 */
void *alloc(size_t size) { return lalloc(size, TRUE); }
 ...
 */
int vim_isspace(int x) { return ((x >= 9 && x <= 13) || x == ' '); }
 ...
 */
int trans_special(char_u **srcp, char_u *dst,
...
int extract_modifiers(int key, int *modp) {
  ...
  if ((modifiers & MOD_MASK_CTRL)
       ...
       * && (Ctrl_chr(key) || key == '?')
       * ???  */
      && strchr("?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_", key) != NULL
#else
 ...
 * argument.
 */
int get_fileformat_force(buf_T *buf, exarg_T *eap) /* can be NULL! */
{
  int c;
 ...
 */
void set_fileformat(int t, int opt_flags) /* OPT_LOCAL and/or OPT_GLOBAL */
{
  char *p = NULL;
