...
#define MAP_HASH(mode, c1)                                                                         \
  (((mode) & (NORMAL + VISUAL + SELECTMODE + OP_PENDING + TERMINAL)) ? (c1) : ((c1) ^ 0x80))
 ...
 * K_SPECIAL and CSI in the returned string are escaped.
 */
char_u *get_inserted(void) { return get_buffcont(&redobuff, FALSE); }

/*
 * Add string "s" after the current block of buffer "buf".
 * K_SPECIAL and CSI should have been escaped already.
 */
static void add_buff(buffheader_T *buf, char_u *s, long slen) /* length of "s" or -1 */
{
  buffblock_T *p;
  long_u len;
  ...
  } else if (buf->bh_curr == NULL) /* buffer has already been read */
  {
    ...
    mch_memmove(buf->bh_first.b_next->b_str, buf->bh_first.b_next->b_str + buf->bh_index,
 ...
 * redbuf2.
 */
int readbuf1_empty(void) { return (readbuf1.bh_first.b_next == NULL); }
#endif

/*
 * Set a typeahead character that won't be flushed.
 */
void typeahead_noflush(int c) { typeahead_char = c; }
 ...
 */
void AppendToRedobuffLit(char_u *str, int len) /* length of "str" or -1 for up to the NUL */
{
  char_u *s = str;
  int c;
  char_u *start;
 ...
 */
void stuffReadbuff(char_u *s) { add_buff(&readbuf1, s, -1L); }

/*
 * Append string "s" to the redo stuff buffer.
 * CSI and K_SPECIAL must already have been escaped.
 */
void stuffRedoReadbuff(char_u *s) { add_buff(&readbuf2, s, -1L); }

void stuffReadbuffLen(char_u *s, long len) { add_buff(&readbuf1, s, len); }
 ...
 * Translates special keys, NUL, CSI, K_SPECIAL and multibyte characters.
 */
void stuffcharReadbuff(int c) { add_char_buff(&readbuf1, c); }

/*
 * Append a number to the stuff buffer.
 */
void stuffnumReadbuff(long n) { add_num_buff(&readbuf1, n); }
...
void stop_redo_ins(void) { block_redo = FALSE; }
...
int ins_typebuf(char_u *str, int noremap, int offset, int nottyped, int silent) {
  ...
  if (offset == 0 && addlen <= typebuf.tb_off) {
     ...
     * the end */
    mch_memmove(s1 + newoff + offset + addlen, typebuf.tb_buf + typebuf.tb_off + offset,
    ...
    mch_memmove(s2 + newoff + offset + addlen, typebuf.tb_noremap + typebuf.tb_off + offset,
 ...
 * that was just added.
 */
int typebuf_changed(int tb_change_cnt) /* old value of typebuf.tb_change_cnt */
{
 ...
 */
int typebuf_typed(void) { return typebuf.tb_maplen == 0; }

/*
 * Return the number of characters that are mapped (or not typed).
 */
int typebuf_maplen(void) { return typebuf.tb_maplen; }
...
void del_typebuf(int len, int offset) {
   ...
   */
    ...
    if (typebuf.tb_off > MAXMAPLEN) {
      ...
      mch_memmove(typebuf.tb_noremap + MAXMAPLEN, typebuf.tb_noremap + typebuf.tb_off,
                  (size_t)offset);
      typebuf.tb_off = MAXMAPLEN;
    }
    /* adjust typebuf.tb_buf (include the NUL at the end) */
    mch_memmove(typebuf.tb_buf + typebuf.tb_off + offset, typebuf.tb_buf + i + len,
                (size_t)(typebuf.tb_len - offset + 1));
    /* adjust typebuf.tb_noremap[] */
    mch_memmove(typebuf.tb_noremap + typebuf.tb_off + offset, typebuf.tb_noremap + i + len,
                (size_t)(typebuf.tb_len - offset));
  }
 ...
 */
void openscript(char_u *name, int directly) /* when TRUE execute directly */
{
    ...
    if (c != NUL && !got_int) {
      ...
      for (;;) {
        ...
        if (got_int) {
          ...
          if (no_mapping == 0 && maphash_valid && (no_zero_mapping == 0 || c1 != '0') &&
              (typebuf.tb_maplen == 0 ||
               (p_remap && (typebuf.tb_noremap[typebuf.tb_off] & (RM_NONE | RM_ABBR)) == 0)) &&
              !(p_paste && (State & (INSERT + CMDLINE))) &&
              !(State == HITRETURN && (c1 == CAR || c1 == ' ')) && State != ASKMORE &&
              State != CONFIRM) {
#ifdef FEAT_LANGMAP
            if (c1 == K_SPECIAL)
              nolmaplen = 2;
            ...
            for (; mp != NULL;
               ...
               */
              if (mp->m_keys[0] == c1 && (mp->m_mode & local_State) &&
                ...
                if (mlen == keylen || (mlen == typebuf.tb_len && typebuf.tb_len < keylen)) {
                  ...
                  if (*s == RM_SCRIPT && (mp->m_keys[0] != K_SPECIAL || mp->m_keys[1] != KS_EXTRA ||
...
#define save_m_silent mp->m_silent
...
#ifdef FEAT_EVAL
                  STRNCMP(s, save_m_keys != NULL ? save_m_keys : mp->m_keys, (size_t)keylen)
#else
         ...
         */
        c = 0;
        if (advance && typebuf.tb_len == 1 && typebuf.tb_buf[typebuf.tb_off] == ESC &&
            !no_mapping && ex_normal_busy == 0 && typebuf.tb_maplen == 0 && (State & INSERT) &&
        ...
        if (((State & INSERT) != 0 || p_lz) && (State & CMDLINE) == 0 && advance &&
 ...
 */
static int inchar(char_u *buf, int maxlen, long wait_time) /* milli seconds */
{
 ...
 */
int do_map(int maptype, char_u *arg, int mode, int abbrev) /* not a mapping but an abbreviation */
{
  char_u *keys;
  mapblock_T *mp, **mpp;
  char_u *rhs;
  ...
  if (map_table == curbuf->b_maphash && haskey && hasarg && maptype != 1) {
    ...
    for (hash = 0; hash < 256 && !got_int; ++hash) {
      ...
      for (; mp != NULL && !got_int; mp = mp->m_next) {
        ...
        if ((mp->m_mode & mode) != 0 && mp->m_keylen == len && unique &&
            STRNCMP(mp->m_keys, keys, (size_t)len) == 0) {
          if (abbrev)
            semsg(_("E224: global abbreviation already exists for %s"), mp->m_keys);
          else
 ...
 */
void map_clear(char_u *cmdp, char_u *arg UNUSED, int forceit, int abbr) {
  int mode;
#ifdef FEAT_LOCALMAP
  int local;
 ...
 */
void map_clear_int(buf_T *buf UNUSED, /* buffer for local mappings */
...
static void showmap(mapblock_T *mp, int local) /* TRUE for buffer-local map */
{
  int len = 1;
  char_u *mapchars;
...
int makemap(FILE *fd, buf_T *buf) /* buffer for local mappings or NULL */
{
    ...
    for (hash = 0; hash < 256; ++hash) {
      ...
      for (; mp; mp = mp->m_next) {
        ...
        do /* do this twice if c2 is set, 3 times with c3 */
        {
              ...
              for (i = 0; i < 2; ++i)
                for (p = (i ? mp->m_str : mp->m_keys); *p; ++p)
                  if (*p == K_SPECIAL || *p == NL)
                    did_cpo = TRUE;
            if (did_cpo) {
              if (fprintf(fd, "let s:cpo_save=&cpo") < 0 || put_eol(fd) < 0 ||
                  fprintf(fd, "set cpo&vim") < 0 || put_eol(fd) < 0)
                return FAIL;
            }
          }
          if (c1 && putc(c1, fd) < 0)
          ...
          if (putc(' ', fd) < 0 || put_escstr(fd, mp->m_keys, 0) == FAIL || putc(' ', fd) < 0 ||
              put_escstr(fd, mp->m_str, 1) == FAIL || put_eol(fd) < 0)
            return FAIL;
          c1 = c2;
          c2 = c3;
          c3 = NUL;
  ...
  if (did_cpo)
    if (fprintf(fd, "let &cpo=s:cpo_save") < 0 || put_eol(fd) < 0 ||
        fprintf(fd, "unlet s:cpo_save") < 0 || put_eol(fd) < 0)
      return FAIL;
  return OK;
}
...
int put_escstr(FILE *fd, char_u *strstart, int what) {
  ...
  for (; *str != NUL; ++str) {
    ...
    if (what == 2 && (VIM_ISWHITE(c) || c == '"' || c == '\\')) {
      if (putc('\\', fd) < 0)
        return FAIL;
    } else if (c < ' ' || c > '~' || c == '|' || (what == 0 && c == ' ') ||
               (what == 1 && str == strstart && c == ' ') || (what != 2 && c == '<')) {
      if (putc(Ctrl_V, fd) < 0)
        return FAIL;
    }
    if (putc(c, fd) < 0)
...
#endif
  {
    for (i = 0; i < (int)(sizeof(cinitmappings) / sizeof(struct initmap)); ++i)
