  ...
  case VAR_BLOB:
    ...
    if (b == NULL || b->bv_ga.ga_len == 0)
      ...
      for (i = 0; i < b->bv_ga.ga_len; i++) {
        if (i > 0)
          ga_concat(gap, (char_u *)",");
        vim_snprintf((char *)numbuf, NUMBUFLEN, "%d", (int)blob_get(b, i));
        ga_concat(gap, numbuf);
      }
      ga_append(gap, ']');
    }
...
static void fill_numbuflen(js_read_T *reader) {
  if (reader->js_fill != NULL &&
      (int)(reader->js_end - reader->js_buf) - reader->js_used < NUMBUFLEN) {
  ...
  if (*p == quote) {
    ...
    if (res != NULL) {
      ...
      if (!enc_utf8) {
        ...
        if (conv.vc_type != CONV_NONE) {
          res->vval.v_string = string_convert(&conv, ga.ga_data, NULL);
          vim_free(ga.ga_data);
        }
        convert_setup(&conv, NULL, NULL);
      } else
