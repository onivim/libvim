    ...
    garray_T *gap) // either &dbg_breakp or &prof_ga
{
  ...
  if ((!here && *p == NUL) || (here && *p != NUL) || (bp->dbg_type == DBG_FUNC && strstr((char *)p, "()") != NULL))
  {
...
void ex_breakdel(exarg_T *eap)
{
  ...
  else
  {
    ...
    {
      bpi = &DEBUGGY(gap, i);
      if (bp->dbg_type == bpi->dbg_type && STRCMP(bp->dbg_name, bpi->dbg_name) == 0 && (bp->dbg_lnum == bpi->dbg_lnum || (bp->dbg_lnum == 0 && (best_lnum == 0 || bpi->dbg_lnum < best_lnum))))
      {
        todel = i;
 ...
 * ":breaklist".
 */
void ex_breaklist(exarg_T *eap UNUSED)
{
  struct debuggy *bp;
  int i;
