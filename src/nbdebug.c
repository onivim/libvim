...
void nbdebug_wait(u_int wait_flags, /* tells what to do */
...
void nbdebug_log_init(char *log_var,   /* env var with log file */
...
static int errorHandler(Display *dpy, XErrorEvent *err) {
  ...
  XGetErrorDatabaseText(dpy, "XRequest", buf, "Unknown", msg, sizeof(msg));
