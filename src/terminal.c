    ...
    int flags)
{
  ...
  else
  {
    ...
    else if (argvar->v_type != VAR_LIST || argvar->vval.v_list == NULL || argvar->vval.v_list->lv_len < 1 || (cmd = tv_get_string_chk(&argvar->vval.v_list->lv_first->li_tv)) == NULL)
      cmd = (char_u *)"";
  ...
  else if (argvar->v_type == VAR_LIST && argvar->vval.v_list != NULL && argvar->vval.v_list->lv_len > 0)
  {
    garray_T ga;
    listitem_T *item;
  ...
  if (argv == NULL && argvar->v_type == VAR_STRING && argvar->vval.v_string != NULL && STRCMP(argvar->vval.v_string, "NONE") == 0)
...
term_job_running_check(term_T *term, int check_job_status)
{
  ...
  if (term != NULL && term->tl_job != NULL && channel_is_open(term->tl_job->jv_channel))
  {
    ...
    if (check_job_status)
      job_status(job);
    return (job->jv_status == JOB_STARTED || (job->jv_channel != NULL && job->jv_channel->ch_keep_open));
  }
  return FALSE;
}
...
may_move_terminal_to_buffer(term_T *term, int redraw)
{
  ...
  /* Update the snapshot only if something changes or the buffer does not
     * have all the lines. */
  if (term->tl_dirty_snapshot || term->tl_buffer->b_ml.ml_line_count <= term->tl_scrollback_scrolled)
...
{
  if (!cursor_color_equal(last_set_cursor_color, desired_cursor_color) || last_set_cursor_shape != desired_cursor_shape || last_set_cursor_blink != desired_cursor_blink)
  {
...
term_use_loop_check(int check_job_status)
{
  ...
  return term != NULL && !term->tl_normal_mode && term->tl_vterm != NULL && term_job_running_check(term, check_job_status);
}
...
int terminal_loop(int blocking)
{
  ...
  int tty_fd = curbuf->b_term->tl_job->jv_channel
                   ->ch_part[get_tty_part(curbuf->b_term)]
                   .ch_fd;
#endif
...
handle_drop_command(listitem_T *item)
{
  ...
  if (opt_item != NULL && opt_item->li_tv.v_type == VAR_DICT && opt_item->li_tv.vval.v_dict != NULL)
  {
...
{
  return a->red == b->red && a->green == b->green && a->blue == b->blue && a->ansi_index == b->ansi_index;
}
...
void f_term_dumpwrite(typval_T *argvars, typval_T *rettv UNUSED)
{
  ...
  for (pos.row = 0; (max_height == 0 || pos.row < max_height) && pos.row < term->tl_rows; ++pos.row)
  {
    int repeat = 0;

    for (pos.col = 0; (max_width == 0 || pos.col < max_width) && pos.col < term->tl_cols; ++pos.col)
    {
      VTermScreenCell cell;
      int same_attr;
      int same_chars = TRUE;
      ...
      same_attr = vtermAttr2hl(cell.attrs) == vtermAttr2hl(prev_cell.attrs) && same_color(&cell.fg, &prev_cell.fg) && same_color(&cell.bg, &prev_cell.bg);
...
term_load_dump(typval_T *argvars, typval_T *rettv, int do_diff)
{
  ...
  init_job_options(&opt);
  if (argvars[do_diff ? 2 : 1].v_type != VAR_UNKNOWN && get_job_options(&argvars[do_diff ? 2 : 1], &opt, 0,
                                                                        JO2_TERM_NAME + JO2_TERM_COLS + JO2_TERM_ROWS + JO2_VERTICAL + JO2_CURWIN + JO2_NORESTORE) == FAIL)
    goto theend;
      ...
      else
      {
        ...
        for (col = 0; col < width && *p1 != NUL && *p2 != NUL; ++col)
        {
          ...
          if (len1 != len2 || STRNCMP(p1, p2, len1) != 0)
            /* text differs */
            textline[col] = 'X';
          else if (lnum == cursor_pos1.row + 1 && col == cursor_pos1.col && (cursor_pos1.row != cursor_pos2.row || cursor_pos1.col != cursor_pos2.col))
            /* cursor in first but not in second */
            textline[col] = '>';
          else if (lnum == cursor_pos2.row + 1 && col == cursor_pos2.col && (cursor_pos1.row != cursor_pos2.row || cursor_pos1.col != cursor_pos2.col))
          ...
          else if (cellattr1 != NULL && cellattr2 != NULL)
          {
            if ((cellattr1 + col)->width != (cellattr2 + col)->width)
              ...
              textline[col] = 'f';
                                 ...
                                 &(cellattr2 + col)->bg))
              textline[col] = 'b';
            else if (vtermAttr2hl((cellattr1 + col)->attrs) != vtermAttr2hl(((cellattr2 + col)->attrs)))
              textline[col] = 'a';
          }
          p1 += len1;
          p2 += len2;
...
int term_swap_diff()
{
  ...
  if (term == NULL || !term_is_finished(curbuf) || term->tl_top_diff_rows == 0 || term->tl_scrollback.ga_len == 0)
    return FAIL;
  ...
  else
  {
    ...
    if (temp != NULL)
    {
      ...
      mch_memmove((sb_line_T *)term->tl_scrollback.ga_data + line_count - top_rows,
                  temp,
...
void f_term_start(typval_T *argvars, typval_T *rettv)
{
  ...
  init_job_options(&opt);
  if (argvars[1].v_type != VAR_UNKNOWN && get_job_options(&argvars[1], &opt,
                                                          JO_TIMEOUT_ALL + JO_STOPONEXIT + JO_CALLBACK + JO_OUT_CALLBACK + JO_ERR_CALLBACK + JO_EXIT_CB + JO_CLOSE_CALLBACK + JO_OUT_IO,
                                                          JO2_TERM_NAME + JO2_TERM_FINISH + JO2_HIDDEN + JO2_TERM_OPENCMD + JO2_TERM_COLS + JO2_TERM_ROWS + JO2_VERTICAL + JO2_CURWIN + JO2_CWD + JO2_ENV + JO2_EOF_CHARS + JO2_NORESTORE + JO2_TERM_KILL + JO2_ANSI_COLORS + JO2_TTY_TYPE) == FAIL)
    return;
...
HRESULT(WINAPI *pCreatePseudoConsole)
(COORD, HANDLE, HANDLE, DWORD, HPCON *);
HRESULT(WINAPI *pResizePseudoConsole)
(HPCON, COORD);
HRESULT(WINAPI *pClosePseudoConsole)
(HPCON);
    ...
    jobopt_T *orig_opt)
{
  ...
  if (!CreateProcessW(NULL, cmd_wchar_copy, NULL, NULL, FALSE,
                      EXTENDED_STARTUPINFO_PRESENT | CREATE_UNICODE_ENVIRONMENT | CREATE_SUSPENDED | CREATE_NEW_PROCESS_GROUP | CREATE_DEFAULT_ERROR_MODE,
                      env_wchar, cwd_wchar,
