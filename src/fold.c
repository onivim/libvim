...
void foldMoveRange(garray_T *gap, linenr_T line1, linenr_T line2, linenr_T dest)
{
  ...
  if (at_start)
  {
    ...
    else if (fold_end(fp) > line2)
    {
      /* Case 3
	     ...
	     */
      foldMarkAdjustRecurse(&fp->fd_nested, line1 - fp->fd_top, line2 - fp->fd_top, MAXLNUM, -range_len);
      fp->fd_len -= range_len;
    }
    else
  ...
  else if (fold_end(fp) > dest)
  {
    ...
    foldMarkAdjustRecurse(&fp->fd_nested, line2 + 1 - fp->fd_top, dest - fp->fd_top, MAXLNUM, -move_len);
    fp->fd_len -= move_len;
    fp->fd_top += move_len;
    return;
  }
...
int put_folds(FILE *fd, win_T *wp)
{
  ...
  {
    if (put_line(fd, "silent! normal! zE") == FAIL || put_folds_recurse(fd, &wp->w_folds, (linenr_T)0) == FAIL)
      return FAIL;
  }
  ...
  for (i = 0; i < gap->ga_len; i++)
  {
    if (fp->fd_flags != FD_LEVEL)
    {
      if (fp->fd_nested.ga_len > 0)
      {
        ...
        if (fprintf(fd, "%ld", fp->fd_top + off) < 0 || put_eol(fd) == FAIL || put_line(fd, "normal! zo") == FAIL)
          return FAIL;
      ...
      else
      {
        ...
        if ((fp->fd_flags == FD_CLOSED && wp->w_p_fdl >= level) || (fp->fd_flags != FD_CLOSED && wp->w_p_fdl < level))
          if (put_fold_open_close(fd, fp, off) == FAIL)
            return FAIL;
      }
    }
...
{
  if (fprintf(fd, "%ld", fp->fd_top + off) < 0 || put_eol(fd) == FAIL || fprintf(fd, "normal! z%c", fp->fd_flags == FD_CLOSED ? 'c' : 'o') < 0 || put_eol(fd) == FAIL)
    return FAIL;
