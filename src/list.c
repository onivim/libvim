 ...
 */
list_T *list_alloc_id(alloc_id_T id UNUSED) {
#ifdef FEAT_EVAL
 ...
 */
int rettv_list_alloc_id(typval_T *rettv, alloc_id_T id UNUSED) {
#ifdef FEAT_EVAL
  if (alloc_fail_id == id && alloc_does_fail(sizeof(list_T)))
    return FAIL;
#endif
 ...
 */
listitem_T *listitem_alloc(void) { return ALLOC_ONE(listitem_T); }
 ...
 */
int list_equal(list_T *l1, list_T *l2, int ic, /* ignore case for strings */
