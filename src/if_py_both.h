...
#define RAISE_INVALID_INDEX_TYPE(idx)                                                              \
  PyErr_FORMAT(PyExc_TypeError, N_("index must be int or slice, not %s"), Py_TYPE_NAME(idx));
 ...
 */
static char_u *StringToChars(PyObject *obj, PyObject **todecref) {
  char_u *str;
  ...
  if (PyBytes_Check(obj)) {
    ...
    *todecref = bytes;
    ...
    PyErr_FORMAT(PyExc_TypeError, N_("expected str() or unicode() instance, but got %s"),
                 Py_TYPE_NAME(obj));
#else
    PyErr_FORMAT(PyExc_TypeError, N_("expected bytes() or str() instance, but got %s"),
                 Py_TYPE_NAME(obj));
#endif
    return NULL;
  }
...
static int NumberToLong(PyObject *obj, long *result, int flags) {
#if PY_MAJOR_VERSION < 3
  if (PyInt_Check(obj)) {
  ...
  if (flags & NUMBER_INT) {
    if (*result > INT_MAX) {
      PyErr_SET_STRING(PyExc_OverflowError, N_("value is too large to fit into C int type"));
      return -1;
    } else if (*result < INT_MIN) {
      PyErr_SET_STRING(PyExc_OverflowError, N_("value is too small to fit into C int type"));
      return -1;
    }
  }
  ...
  if (flags & NUMBER_NATURAL) {
    if (*result <= 0) {
      PyErr_SET_STRING(PyExc_ValueError, N_("number must be greater than zero"));
      return -1;
    }
  } else if (flags & NUMBER_UNSIGNED) {
    if (*result < 0) {
      PyErr_SET_STRING(PyExc_ValueError, N_("number must be greater or equal to zero"));
      return -1;
    }
  }
...
static int add_string(PyObject *list, char *s) {
  PyObject *string;
...
static PyObject *ObjectDir(PyObject *self, char **attributes) {
  PyMethodDef *method;
  char **attr;
  PyObject *ret;
...
static char *OutputAttrs[] = {"softspace", NULL};

static PyObject *OutputDir(PyObject *self) { return ObjectDir(self, OutputAttrs); }

static int OutputSetattr(OutputObject *self, char *name, PyObject *valObject) {
  if (valObject == NULL) {
    PyErr_SET_STRING(PyExc_AttributeError, N_("can't delete OutputObject attributes"));
    return -1;
  }
...
static int write_output(OutputObject *self, PyObject *string) {
  ...
  Py_BEGIN_ALLOW_THREADS Python_Lock_Vim();
  if (error)
    emsg_severe = TRUE;
  writer((writefn)(error ? emsg : msg), (char_u *)str, len);
  Python_Release_Vim();
  Py_END_ALLOW_THREADS PyMem_Free(str);

  return 0;
}

static PyObject *OutputWrite(OutputObject *self, PyObject *string) {
...
static PyObject *OutputWritelines(OutputObject *self, PyObject *seq) {
  PyObject *iterator;
  PyObject *item;
...
static PyObject *AlwaysNone(PyObject *self UNUSED) {
  /* do nothing */
  Py_INCREF(Py_None);
  return Py_None;
}

static PyObject *AlwaysFalse(PyObject *self UNUSED) {
  /* do nothing */
...
static PyObject *AlwaysTrue(PyObject *self UNUSED) {
  /* do nothing */
...
static OutputObject Output = {PyObject_HEAD_INIT(&OutputType) 0, 0};

static OutputObject Error = {PyObject_HEAD_INIT(&OutputType) 0, 1};

static int PythonIO_Init_io(void) {
...
static void LoaderDestructor(LoaderObject *self) {
  vim_free(self->fullname);
  Py_XDECREF(self->result);
  DESTRUCTOR_FINISH(self);
}

static PyObject *LoaderLoadModule(LoaderObject *self, PyObject *args UNUSED) {
...
static void VimTryStart(void) { ++trylevel; }
...
static int VimCheckInterrupt(void) {
  if (got_int) {
    PyErr_SetNone(PyExc_KeyboardInterrupt);
    return 1;
  }
...
static PyObject *VimCommand(PyObject *self UNUSED, PyObject *string) {
  char_u *cmd;
  PyObject *ret;
  PyObject *todecref;
 ...
 */
static PyObject *VimToPython(typval_T *our_tv, int depth, PyObject *lookup_dict) {
  PyObject *ret;
  PyObject *newObj;
  ...
  if ((our_tv->v_type == VAR_LIST && our_tv->vval.v_list != NULL) ||
    ...
    sprintf(ptrBuf, "%p",
            our_tv->v_type == VAR_LIST ? (void *)our_tv->vval.v_list : (void *)our_tv->vval.v_dict);
  ...
  if (our_tv->v_type == VAR_STRING)
    ret = PyString_FromString(our_tv->vval.v_string == NULL ? "" : (char *)our_tv->vval.v_string);
    ...
    ret = PyBytes_FromStringAndSize((char *)our_tv->vval.v_blob->bv_ga.ga_data,
...
static PyObject *VimEvalPy(PyObject *self UNUSED, PyObject *string) {
  typval_T *our_tv;
  PyObject *ret;
  char_u *expr;
  PyObject *todecref;
...
static PyObject *VimStrwidth(PyObject *self UNUSED, PyObject *string) {
  char_u *str;
  PyObject *todecref;
  int len;
...
static PyObject *_VimChdir(PyObject *_chdir, PyObject *args, PyObject *kwargs) {
  PyObject *ret;
  PyObject *newwd;
  PyObject *todecref;
  char_u *new_dir;
...
static PyObject *VimFchdir(PyObject *self UNUSED, PyObject *args, PyObject *kwargs) {
...
static void map_rtp_callback(char_u *path, void *_data) {
  ...
  mr_data->result = PyObject_CallFunctionObjArgs(mr_data->callable, pathObject, NULL);
...
static PyObject *VimForeachRTP(PyObject *self UNUSED, PyObject *callable) {
  map_rtp_data data;
...
static void map_finder_callback(char_u *path, void *_data) {
  void **data = (void **)_data;
...
#define PYTHONX_STRING_LENGTH 7 /* STRLEN("pythonx") */
  if (!(pathbuf = PyMem_New(char, pathlen + STRLEN(PATHSEPSTR) + PYTHONX_STRING_LENGTH + 1))) {
    PyErr_NoMemory();
    *data = NULL;
    return;
  }
  ...
  mch_memmove(pathbuf + pathlen, PY_MAIN_DIR_STRING, PYTHONX_STRING_LENGTH + 1);
  ...
  mch_memmove(pathbuf + pathlen, PY_ALTERNATE_DIR_STRING, PYTHONX_STRING_LENGTH + 1);
...
static PyObject *Vim_GetPaths(PyObject *self UNUSED) {
  PyObject *ret;
...
#if PY_VERSION_HEX >= 0x030700f0
static PyObject *FinderFindSpec(PyObject *self, PyObject *args) {
  char *fullname;
  PyObject *paths;
...
static PyObject *FinderFindModule(PyObject *self UNUSED, PyObject *args UNUSED) {
...
static PyObject *call_load_module(char *name, int len, PyObject *find_module_result) {
  ...
  if (!PyTuple_Check(find_module_result)) {
    PyErr_FORMAT(PyExc_TypeError, N_("expected 3-tuple as imp.find_module() result, but got %s"),
  ...
  return PyObject_CallFunction(py_load_module, "s#OOO", name, len, fd, pathname, description);
}

static PyObject *find_module(char *fullname, char *tail, PyObject *new_path) {
  ...
  if ((dot = (char *)vim_strchr((char_u *)tail, '.'))) {
    ...
    if (!(find_module_result =
              PyObject_CallFunction(py_find_module, "s#O", tail, partlen, new_path))) {
    ...
    if (!(module =
              call_load_module(fullname, ((int)(tail - fullname)) + partlen, find_module_result))) {
    ...
    if (!(find_module_result = PyObject_CallFunction(py_find_module, "sO", tail, new_path))) {
      if (PyErr_Occurred() && PyErr_ExceptionMatches(PyExc_ImportError))
        PyErr_Clear();
      return NULL;
    }
...
static PyObject *FinderFindModule(PyObject *self, PyObject *args) {
  char *fullname;
  PyObject *result;
  PyObject *new_path;
  LoaderObject *loader;
...
static PyObject *VimPathHook(PyObject *self UNUSED, PyObject *args) {
  char *path;
...
static PyObject *IterNew(void *start, destructorfun destruct, nextfun next, traversefun traverse,
                         clearfun clear) {
  IterObject *self;
...
static void IterDestructor(IterObject *self) {
  PyObject_GC_UnTrack((void *)(self));
  self->destruct(self->cur);
  PyObject_GC_Del((void *)(self));
}

static int IterTraverse(IterObject *self, visitproc visit, void *arg) {
...
static int IterClear(IterObject *self) {
  if (self->clear != NULL)
...
static PyObject *IterNext(IterObject *self) { return self->next(&self->cur); }

static PyObject *IterIter(PyObject *self) {
  Py_INCREF(self);
  return self;
}
...
static void pyll_remove(pylinkedlist_T *ref, pylinkedlist_T **last) {
...
static void pyll_add(PyObject *self, pylinkedlist_T *ref, pylinkedlist_T **last) {
  if (*last == NULL)
    ref->pll_prev = NULL;
  else {
    (*last)->pll_next = ref;
...
static PyObject *DictionaryNew(PyTypeObject *subtype, dict_T *dict) {
...
static PyObject *DictionaryConstructor(PyTypeObject *subtype, PyObject *args, PyObject *kwargs) {
  DictionaryObject *self;
  dict_T *dict;
...
static void DictionaryDestructor(DictionaryObject *self) {
...
static char *DictionaryAttrs[] = {"locked", "scope", NULL};

static PyObject *DictionaryDir(PyObject *self) { return ObjectDir(self, DictionaryAttrs); }

static int DictionarySetattr(DictionaryObject *self, char *name, PyObject *valObject) {
  if (valObject == NULL) {
    PyErr_SET_STRING(PyExc_AttributeError, N_("cannot delete vim.Dictionary attributes"));
    return -1;
  }

  if (strcmp(name, "locked") == 0) {
    if (self->dict->dv_lock == VAR_FIXED) {
      PyErr_SET_STRING(PyExc_TypeError, N_("cannot modify fixed dictionary"));
      return -1;
    } else {
...
static PyInt DictionaryLength(DictionaryObject *self) {
...
static PyObject *_DictionaryItem(DictionaryObject *self, PyObject *args, int flags) {
  PyObject *keyObject;
...
static PyObject *DictionaryItem(DictionaryObject *self, PyObject *keyObject) {
  return _DictionaryItem(self, keyObject, 0);
}

static int DictionaryContains(DictionaryObject *self, PyObject *keyObject) {
...
static PyObject *DictionaryIterNext(dictiterinfo_T **dii) {
  PyObject *ret;

  if (!(*dii)->todo)
    return NULL;

  if ((*dii)->ht->ht_array != (*dii)->ht_array || (*dii)->ht->ht_used != (*dii)->ht_used) {
    PyErr_SET_STRING(PyExc_RuntimeError, N_("hashtab changed during iteration"));
    return NULL;
  }
...
static PyObject *DictionaryIter(DictionaryObject *self) {
  dictiterinfo_T *dii;
  hashtab_T *ht;
  ...
  return IterNew(dii, (destructorfun)PyMem_Free, (nextfun)DictionaryIterNext, NULL, NULL);
}

static PyInt DictionaryAssItem(DictionaryObject *self, PyObject *keyObject, PyObject *valObject) {
  char_u *key;
  typval_T tv;
  dict_T *dict = self->dict;
  dictitem_T *di;
...
static PyObject *DictionaryListObjects(DictionaryObject *self, hi_to_py hiconvert) {
  dict_T *dict = self->dict;
...
static PyObject *dict_key(hashitem_T *hi) { return PyBytes_FromString((char *)(hi->hi_key)); }

static PyObject *DictionaryListKeys(DictionaryObject *self) {
  return DictionaryListObjects(self, dict_key);
}

static PyObject *dict_val(hashitem_T *hi) {
  dictitem_T *di;
...
static PyObject *DictionaryListValues(DictionaryObject *self) {
  return DictionaryListObjects(self, dict_val);
}

static PyObject *dict_item(hashitem_T *hi) {
  PyObject *keyObject;
  PyObject *valObject;
  PyObject *ret;
...
static PyObject *DictionaryListItems(DictionaryObject *self) {
  return DictionaryListObjects(self, dict_item);
}

static PyObject *DictionaryUpdate(DictionaryObject *self, PyObject *args, PyObject *kwargs) {
  dict_T *dict = self->dict;
...
static PyObject *DictionaryGet(DictionaryObject *self, PyObject *args) {
  return _DictionaryItem(self, args, DICT_FLAG_HAS_DEFAULT | DICT_FLAG_NONE_DEFAULT);
}

static PyObject *DictionaryPop(DictionaryObject *self, PyObject *args) {
  return _DictionaryItem(self, args, DICT_FLAG_HAS_DEFAULT | DICT_FLAG_POP);
}

static PyObject *DictionaryPopItem(DictionaryObject *self) {
  hashitem_T *hi;
  PyObject *ret;
  PyObject *valObject;
  dictitem_T *di;
...
static PyObject *DictionaryHasKey(DictionaryObject *self, PyObject *keyObject) {
...
static PyObject *ListNew(PyTypeObject *subtype, list_T *list) {
  ListObject *self;
...
static int list_py_concat(list_T *l, PyObject *obj, PyObject *lookup_dict) {
  PyObject *iterator;
  PyObject *item;
  listitem_T *li;
...
static PyObject *ListConstructor(PyTypeObject *subtype, PyObject *args, PyObject *kwargs) {
  list_T *list;
  PyObject *obj = NULL;

  if (kwargs) {
    PyErr_SET_STRING(PyExc_TypeError, N_("list constructor does not accept keyword arguments"));
    return NULL;
  }
...
static void ListDestructor(ListObject *self) {
...
static PyInt ListLength(ListObject *self) { return ((PyInt)(self->list->lv_len)); }

static PyObject *ListIndex(ListObject *self, Py_ssize_t index) {
  listitem_T *li;
  ...
  if (li == NULL) {
    ...
    PyErr_VIM_FORMAT(N_("internal error: failed to get vim list item %d"), (int)index);
    return NULL;
  }
  return ConvertToPyObject(&li->li_tv);
}

static PyObject *ListSlice(ListObject *self, Py_ssize_t first, Py_ssize_t step,
                           Py_ssize_t slicelen) {
  PyInt i;
  PyObject *list;
...
static PyObject *ListItem(ListObject *self, PyObject *idx) {
#if PY_MAJOR_VERSION < 3
  if (PyInt_Check(idx)) {
    ...
    if (PySlice_GetIndicesEx((PySliceObject_T *)idx, ListLength(self), &start, &stop, &step,
                             &slicelen) < 0)
      return NULL;
...
static void list_restore(Py_ssize_t numadded, Py_ssize_t numreplaced, Py_ssize_t slicelen,
...
static int ListAssSlice(ListObject *self, Py_ssize_t first, Py_ssize_t step, Py_ssize_t slicelen,
                        PyObject *obj) {
  PyObject *iterator;
  PyObject *item;
  listitem_T *li;
  ...
  if (first == size)
    ...
    if (li == NULL) {
      PyErr_VIM_FORMAT(N_("internal error: no vim list item %d"), (int)first);
      if (obj != NULL)
        PyMem_Free(lis);
      return -1;
    }
...
static int ListAssIndex(ListObject *self, Py_ssize_t index, PyObject *obj) {
  typval_T tv;
  list_T *l = self->list;
  listitem_T *li;
...
static Py_ssize_t ListAssItem(ListObject *self, PyObject *idx, PyObject *obj) {
#if PY_MAJOR_VERSION < 3
  if (PyInt_Check(idx)) {
    long _idx = PyInt_AsLong(idx);
    ...
    if (PySlice_GetIndicesEx((PySliceObject_T *)idx, ListLength(self), &start, &stop, &step,
                             &slicelen) < 0)
      return -1;
    return ListAssSlice(self, start, step, slicelen, obj);
  } else {
    RAISE_INVALID_INDEX_TYPE(idx);
    return -1;
  }
...
static PyObject *ListConcatInPlace(ListObject *self, PyObject *obj) {
  list_T *l = self->list;
  PyObject *lookup_dict;
...
static void ListIterDestruct(listiterinfo_T *lii) {
  list_rem_watch(lii->list, &lii->lw);
  PyMem_Free(lii);
}

static PyObject *ListIterNext(listiterinfo_T **lii) {
  PyObject *ret;
...
static PyObject *ListIter(ListObject *self) {
  listiterinfo_T *lii;
  list_T *l = self->list;
  ...
  return IterNew(lii, (destructorfun)ListIterDestruct, (nextfun)ListIterNext, NULL, NULL);
}

static char *ListAttrs[] = {"locked", NULL};

static PyObject *ListDir(PyObject *self) { return ObjectDir(self, ListAttrs); }

static int ListSetattr(ListObject *self, char *name, PyObject *valObject) {
  if (valObject == NULL) {
    PyErr_SET_STRING(PyExc_AttributeError, N_("cannot delete vim.List attributes"));
    return -1;
  }
...
static struct PyMethodDef ListMethods[] = {{"extend", (PyCFunction)ListConcatInPlace, METH_O, ""},
...
static PyObject *FunctionNew(PyTypeObject *subtype, char_u *name, int argc, typval_T *argv,
  ...
  if (isdigit(*name)) {
    if (!translated_function_exists(name)) {
      PyErr_FORMAT(PyExc_ValueError, N_("unnamed function %s does not exist"), name);
      return NULL;
    }
    ...
    if ((p = get_expanded_name(name, vim_strchr(name, AUTOLOAD_CHAR) == NULL)) == NULL) {
      PyErr_FORMAT(PyExc_ValueError, N_("function %s does not exist"), name);
      return NULL;
    }
...
static PyObject *FunctionConstructor(PyTypeObject *subtype, PyObject *args, PyObject *kwargs) {
  PyObject *self;
...
static void FunctionDestructor(FunctionObject *self) {
  int i;
...
static char *FunctionAttrs[] = {"softspace", "args", "self", "auto_rebind", NULL};

static PyObject *FunctionDir(PyObject *self) { return ObjectDir(self, FunctionAttrs); }

static PyObject *FunctionAttr(FunctionObject *self, char *name) {
  list_T *list;
  int i;
  if (strcmp(name, "name") == 0)
    ...
    return self->self == NULL ? AlwaysNone(NULL) : NEW_DICTIONARY(self->self);
  else if (strcmp(name, "auto_rebind") == 0)
    return self->auto_rebind ? AlwaysTrue(NULL) : AlwaysFalse(NULL);
 ...
 */
static void set_partial(FunctionObject *self, partial_T *pt, int exported) {
  int i;
...
static PyObject *FunctionCall(FunctionObject *self, PyObject *argsObject, PyObject *kwargs) {
...
static PyObject *FunctionRepr(FunctionObject *self) {
  PyObject *ret;
  garray_T repr_ga;
  int i;
  char_u *tofree = NULL;
  ...
  if (self->argv) {
    ...
    for (i = 0; i < self->argc; i++) {
      ...
      ga_concat(&repr_ga, tv2string(&self->argv[i], &tofree, numbuf, get_copyID()));
      vim_free(tofree);
    }
    --emsg_silent;
    ga_append(&repr_ga, ']');
...
static struct PyMethodDef FunctionMethods[] = {
    {"__dir__", (PyCFunction)FunctionDir, METH_NOARGS, ""}, {NULL, NULL, 0, NULL}};
...
static int dummy_check(void *arg UNUSED) { return 0; }

static PyObject *OptionsNew(int opt_type, void *from, checkfun Check, PyObject *fromObj) {
  OptionsObject *self;
...
static void OptionsDestructor(OptionsObject *self) {
  PyObject_GC_UnTrack((void *)(self));
  Py_XDECREF(self->fromObj);
  PyObject_GC_Del((void *)(self));
}

static int OptionsTraverse(OptionsObject *self, visitproc visit, void *arg) {
  Py_VISIT(self->fromObj);
  return 0;
}

static int OptionsClear(OptionsObject *self) {
  Py_CLEAR(self->fromObj);
  return 0;
}

static PyObject *OptionsItem(OptionsObject *self, PyObject *keyObject) {
  char_u *key;
  int flags;
  long numval;
  char_u *stringval;
  ...
  flags = get_option_value_strict(key, &numval, &stringval, self->opt_type, self->from);
  ...
  if (flags & SOPT_UNSET) {
    ...
    if (stringval) {
      ...
      PyErr_SET_STRING(PyExc_RuntimeError, N_("unable to get option value"));
      return NULL;
    }
  } else {
...
static PyObject *OptionsIterNext(optiterinfo_T **oii) {
  char_u *name;
...
static PyObject *OptionsIter(OptionsObject *self) {
  optiterinfo_T *oii;
  ...
  return IterNew(oii, (destructorfun)PyMem_Free, (nextfun)OptionsIterNext, NULL, NULL);
}

static int set_option_value_err(char_u *key, int numval, char_u *stringval, int opt_flags) {
  char *errmsg;
...
static int set_option_value_for(char_u *key, int numval, char_u *stringval, int opt_flags,
  ...
  switch (opt_type) {
  case SREQ_WIN:
    if (switch_win(&save_curwin, &save_curtab, (win_T *)from, win_find_tabpage((win_T *)from),
                   FALSE) == FAIL) {
...
static int OptionsAssItem(OptionsObject *self, PyObject *keyObject, PyObject *valObject) {
  char_u *key;
  int flags;
  int opt_flags;
  int ret = 0;
  ...
  flags = get_option_value_strict(key, NULL, NULL, self->opt_type, self->from);
  ...
  if (valObject == NULL) {
    if (self->opt_type == SREQ_GLOBAL) {
      PyErr_FORMAT(PyExc_ValueError, N_("unable to unset global option %s"), key);
      Py_XDECREF(todecref);
      return -1;
  ...
  if (flags & SOPT_BOOL) {
    ...
    if (istrue == -1)
      ...
      ret = set_option_value_for(key, istrue, NULL, opt_flags, self->opt_type, self->from);
  } else if (flags & SOPT_NUM) {
    long val;
    ...
    ret = set_option_value_for(key, (int)val, NULL, opt_flags, self->opt_type, self->from);
  } else {
    char_u *val;
    PyObject *todecref2;

    if ((val = StringToChars(valObject, &todecref2))) {
      ret = set_option_value_for(key, 0, val, opt_flags, self->opt_type, self->from);
...
static int CheckTabPage(TabPageObject *self) {
...
static PyObject *TabPageNew(tabpage_T *tab) {
  TabPageObject *self;
...
static void TabPageDestructor(TabPageObject *self) {
...
static char *TabPageAttrs[] = {"windows", "number", "vars", "window", "valid", NULL};

static PyObject *TabPageDir(PyObject *self) { return ObjectDir(self, TabPageAttrs); }

static PyObject *TabPageAttrValid(TabPageObject *self, char *name) {
  PyObject *ret;
...
static PyObject *TabPageAttr(TabPageObject *self, char *name) {
...
static PyObject *TabPageRepr(TabPageObject *self) {
  if (self->tab == INVALID_TABPAGE_VALUE)
    ...
    if (t == 0)
      return PyString_FromFormat("<tabpage object (unknown) at %p>", (self));
    else
...
static PyInt TabListLength(PyObject *self UNUSED) {
...
static PyObject *TabListItem(PyObject *self UNUSED, PyInt n) {
  tabpage_T *tp;
...
static int CheckWindow(WindowObject *self) {
...
static PyObject *WindowNew(win_T *win, tabpage_T *tab) {
...
static void WindowDestructor(WindowObject *self) {
  PyObject_GC_UnTrack((void *)(self));
...
static int WindowTraverse(WindowObject *self, visitproc visit, void *arg) {
  Py_VISIT(((PyObject *)(self->tabObject)));
  return 0;
}

static int WindowClear(WindowObject *self) {
  Py_CLEAR(self->tabObject);
  return 0;
}

static win_T *get_firstwin(TabPageObject *tabObject) {
  if (tabObject) {
...
static char *WindowAttrs[] = {"buffer", "cursor",  "height", "row",     "width", "col",
                              "vars",   "options", "number", "tabpage", "valid", NULL};

static PyObject *WindowDir(PyObject *self) { return ObjectDir(self, WindowAttrs); }

static PyObject *WindowAttrValid(WindowObject *self, char *name) {
  PyObject *ret;
...
static PyObject *WindowAttr(WindowObject *self, char *name) {
  if (strcmp(name, "buffer") == 0)
    ...
    return OptionsNew(SREQ_WIN, self->win, (checkfun)CheckWindow, (PyObject *)self);
  else if (strcmp(name, "number") == 0) {
    if (CheckTabPage(self->tabObject))
      return NULL;
    return PyLong_FromLong((long)get_win_number(self->win, get_firstwin(self->tabObject)));
...
static int WindowSetattr(WindowObject *self, char *name, PyObject *valObject) {
  if (CheckWindow(self))
    return -1;
...
static PyObject *WindowRepr(WindowObject *self) {
  if (self->win == INVALID_WINDOW_VALUE)
    ...
    if (w == 0)
      return PyString_FromFormat("<window object (unknown) at %p>", (self));
    else
...
static void WinListDestructor(WinListObject *self) {
...
static PyInt WinListLength(WinListObject *self) {
  win_T *w;
  PyInt n = 0;
...
static PyObject *WinListItem(WinListObject *self, PyInt n) {
  win_T *w;
 ...
 * string object.
 */
static PyObject *GetBufferLine(buf_T *buf, PyInt n) {
 ...
 */
static PyObject *GetBufferLineList(buf_T *buf, PyInt lo, PyInt hi) {
  PyInt i;
  PyInt n = hi - lo;
  ...
  for (i = 0; i < n; ++i) {
    PyObject *string = LineToString((char *)ml_get_buf(buf, (linenr_T)(lo + i), FALSE));
 ...
 */
static int SetBufferLine(buf_T *buf, PyInt n, PyObject *line, PyInt *len_change) {
 ...
 * is set to the change in the buffer length.
 */
static int SetBufferLineList(buf_T *buf, PyInt lo, PyInt hi, PyObject *list, PyInt *len_change) {
  bufref_T save_curbuf = {NULL, 0, 0};
  win_T *save_curwin = NULL;
  tabpage_T *save_curtab = NULL;
  ...
  if (list == Py_None || list == NULL) {
    ...
    for (i = 0; i < new_len; ++i) {
      ...
      if (!(line = PyList_GetItem(list, i)) || !(array[i] = StringToLine(line))) {
        while (i)
          vim_free(array[--i]);
        PyMem_Free(array);
        return FAIL;
 ...
 * is set to the change in the buffer length.
 */
static int InsertBufferLines(buf_T *buf, PyInt n, PyObject *lines, PyInt *len_change) {
  bufref_T save_curbuf = {NULL, 0, 0};
  win_T *save_curwin = NULL;
  tabpage_T *save_curtab = NULL;
  ...
  if (PyBytes_Check(lines) || PyUnicode_Check(lines)) {
    ...
    for (i = 0; i < size; ++i) {
      ...
      if (!(line = PyList_GetItem(lines, i)) || !(array[i] = StringToLine(line))) {
        while (i)
          vim_free(array[--i]);
        PyMem_Free(array);
        return FAIL;
...
static int CheckBuffer(BufferObject *self) {
...
static PyObject *RBItem(BufferObject *self, PyInt n, PyInt start, PyInt end) {
  if (CheckBuffer(self))
    return NULL;
...
static PyObject *RBSlice(BufferObject *self, PyInt lo, PyInt hi, PyInt start, PyInt end) {
  PyInt size;
...
static PyInt RBAsItem(BufferObject *self, PyInt n, PyObject *valObject, PyInt start, PyInt end,
                      PyInt *new_end) {
  PyInt len_change;
...
static PyInt RBAsSlice(BufferObject *self, PyInt lo, PyInt hi, PyObject *valObject, PyInt start,
  ...
  if (SetBufferLineList(self->buf, lo + start, hi + start, valObject, &len_change) == FAIL)
    return -1;
...
static PyObject *RBAppend(BufferObject *self, PyObject *args, PyInt start, PyInt end,
                          PyInt *new_end) {
  PyObject *lines;
  PyInt len_change;
  PyInt max;
...
static PyObject *RangeNew(buf_T *buf, PyInt start, PyInt end) {
  BufferObject *bufr;
  RangeObject *self;
...
static void RangeDestructor(RangeObject *self) {
  PyObject_GC_UnTrack((void *)(self));
  Py_XDECREF(self->buf);
  PyObject_GC_Del((void *)(self));
}

static int RangeTraverse(RangeObject *self, visitproc visit, void *arg) {
  Py_VISIT(((PyObject *)(self->buf)));
  return 0;
}

static int RangeClear(RangeObject *self) {
  Py_CLEAR(self->buf);
  return 0;
}

static PyInt RangeLength(RangeObject *self) {
...
static PyObject *RangeItem(RangeObject *self, PyInt n) {
  return RBItem(self->buf, n, self->start, self->end);
}

static PyObject *RangeSlice(RangeObject *self, PyInt lo, PyInt hi) {
  return RBSlice(self->buf, lo, hi, self->start, self->end);
}

static char *RangeAttrs[] = {"start", "end", NULL};

static PyObject *RangeDir(PyObject *self) { return ObjectDir(self, RangeAttrs); }

static PyObject *RangeAppend(RangeObject *self, PyObject *args) {
  return RBAppend(self->buf, args, self->start, self->end, &self->end);
}

static PyObject *RangeRepr(RangeObject *self) {
  if (self->buf->buf == INVALID_BUFFER_VALUE)
    return PyString_FromFormat("<range object (for deleted buffer) at %p>", (self));
  else {
    ...
    return PyString_FromFormat("<range %s (%d:%d)>", name, (int)self->start, (int)self->end);
  }
}
...
static void BufferDestructor(BufferObject *self) {
...
static PyInt BufferLength(BufferObject *self) {
  /* HOW DO WE SIGNAL AN ERROR FROM THIS FUNCTION? */
  if (CheckBuffer(self))
    return -1; /* ??? */
...
static PyObject *BufferItem(BufferObject *self, PyInt n) { return RBItem(self, n, 1, -1); }

static PyObject *BufferSlice(BufferObject *self, PyInt lo, PyInt hi) {
  return RBSlice(self, lo, hi, 1, -1);
}

static char *BufferAttrs[] = {"name", "number", "vars", "options", "valid", NULL};

static PyObject *BufferDir(PyObject *self) { return ObjectDir(self, BufferAttrs); }

static PyObject *BufferAttrValid(BufferObject *self, char *name) {
  PyObject *ret;
...
static PyObject *BufferAttr(BufferObject *self, char *name) {
  if (strcmp(name, "name") == 0)
    return PyString_FromString((self->buf->b_ffname == NULL ? "" : (char *)self->buf->b_ffname));
  else if (strcmp(name, "number") == 0)
    return Py_BuildValue(Py_ssize_t_fmt, self->buf->b_fnum);
  else if (strcmp(name, "vars") == 0)
    return NEW_DICTIONARY(self->buf->b_vars);
  else if (strcmp(name, "options") == 0)
    return OptionsNew(SREQ_BUF, self->buf, (checkfun)CheckBuffer, (PyObject *)self);
  else if (strcmp(name, "__members__") == 0)
...
static int BufferSetattr(BufferObject *self, char *name, PyObject *valObject) {
  if (CheckBuffer(self))
    return -1;
...
static PyObject *BufferAppend(BufferObject *self, PyObject *args) {
  return RBAppend(self, args, 1, -1, NULL);
}

static PyObject *BufferMark(BufferObject *self, PyObject *pmarkObject) {
  pos_T *posp;
  char_u *pmark;
  char_u mark;
  bufref_T savebuf;
  ...
  if (pmark[0] == '\0' || pmark[1] != '\0') {
    PyErr_SET_STRING(PyExc_ValueError, N_("mark name must be a single character"));
    Py_XDECREF(todecref);
    return NULL;
  }
...
static PyObject *BufferRange(BufferObject *self, PyObject *args) {
  PyInt start;
  PyInt end;
...
static PyObject *BufferRepr(BufferObject *self) {
  if (self->buf == INVALID_BUFFER_VALUE)
...
static struct PyMethodDef BufferMethods[] = {
    ...
    {"mark", (PyCFunction)BufferMark, METH_O,
     "Return (row,col) representing position of named mark"},
    {"range", (PyCFunction)BufferRange, METH_VARARGS,
     "Return a range object which represents the part of the given buffer "
     "between line numbers s and e"},
...
static PyInt BufMapLength(PyObject *self UNUSED) {
  buf_T *b = firstbuf;
  PyInt n = 0;
...
static PyObject *BufMapItem(PyObject *self UNUSED, PyObject *keyObject) {
  buf_T *b;
  long bnr;
...
static void BufMapIterDestruct(PyObject *buffer) {
...
static int BufMapIterTraverse(PyObject *buffer, visitproc visit, void *arg) {
  if (buffer)
    Py_VISIT(buffer);
  return 0;
}

static int BufMapIterClear(PyObject **buffer) {
  if (*buffer)
    Py_CLEAR(*buffer);
  return 0;
}

static PyObject *BufMapIterNext(PyObject **buffer) {
  PyObject *next;
  PyObject *ret;
...
static PyObject *BufMapIter(PyObject *self UNUSED) {
  PyObject *buffer;

  buffer = BufferNew(firstbuf);
  return IterNew(buffer, (destructorfun)BufMapIterDestruct, (nextfun)BufMapIterNext,
...
static char *CurrentAttrs[] = {"buffer", "window", "line", "range", "tabpage", NULL};

static PyObject *CurrentDir(PyObject *self) { return ObjectDir(self, CurrentAttrs); }

static PyObject *CurrentGetattr(PyObject *self UNUSED, char *name) {
  if (strcmp(name, "buffer") == 0)
...
static int CurrentSetattr(PyObject *self UNUSED, char *name, PyObject *valObject) {
  if (strcmp(name, "line") == 0) {
    if (SetBufferLine(curbuf, (PyInt)curwin->w_cursor.lnum, valObject, NULL) == FAIL)
      return -1;
    ...
    if (valObject->ob_type != &BufferType) {
      PyErr_FORMAT(PyExc_TypeError, N_("expected vim.Buffer object, but got %s"),
    ...
    if (valObject->ob_type != &WindowType) {
      PyErr_FORMAT(PyExc_TypeError, N_("expected vim.Window object, but got %s"),
                   Py_TYPE_NAME(valObject));
      return -1;
    }
    ...
    if (!count) {
      PyErr_SET_STRING(PyExc_ValueError, N_("failed to find window in the current tab page"));
      return -1;
    }
    ...
    if (((WindowObject *)(valObject))->win != curwin) {
      if (VimTryEnd())
        return -1;
      PyErr_SET_STRING(PyExc_RuntimeError, N_("did not switch to the specified window"));
      return -1;
    }
    ...
    if (valObject->ob_type != &TabPageType) {
      PyErr_FORMAT(PyExc_TypeError, N_("expected vim.TabPage object, but got %s"),
                   Py_TYPE_NAME(valObject));
      return -1;
    }
    ...
    if (((TabPageObject *)(valObject))->tab != curtab) {
      if (VimTryEnd())
        return -1;
      PyErr_SET_STRING(PyExc_RuntimeError, N_("did not switch to the specified tab page"));
      return -1;
    }
...
static void init_range_cmd(exarg_T *eap) {
  RangeStart = eap->line1;
  RangeEnd = eap->line2;
}

static void init_range_eval(typval_T *rettv UNUSED) {
  RangeStart = (PyInt)curwin->w_cursor.lnum;
  RangeEnd = RangeStart;
}

static void run_cmd(const char *cmd, void *arg UNUSED
#ifdef PY_CAN_RECURSE
                    ,
...
static void run_do(const char *cmd, void *arg UNUSED
#ifdef PY_CAN_RECURSE
                   ,
...
static void run_eval(const char *cmd, typval_T *rettv
#ifdef PY_CAN_RECURSE
                     ,
                     PyGILState_STATE *pygilstate UNUSED
#endif
...
static int set_ref_in_py(const int copyID) {
  pylinkedlist_T *cur;
  dict_T *dd;
  list_T *ll;
  int i;
  ...
  if (lastfunc != NULL) {
    for (cur = lastfunc; !abort && cur != NULL; cur = cur->pll_prev) {
      ...
      if (func->self != NULL && func->self->dv_copyID != copyID) {
        ...
        abort = abort || set_ref_in_ht(&func->self->dv_hashtab, copyID, NULL);
      }
      if (func->argc)
...
static int set_string_copy(char_u *str, typval_T *tv) {
...
static int pydict_to_tv(PyObject *obj, typval_T *tv, PyObject *lookup_dict) {
  dict_T *dict;
  char_u *key;
  dictitem_T *di;
  PyObject *keyObject;
...
static int pymap_to_tv(PyObject *obj, typval_T *tv, PyObject *lookup_dict) {
  dict_T *dict;
  char_u *key;
  dictitem_T *di;
  PyObject *list;
...
static int pyseq_to_tv(PyObject *obj, typval_T *tv, PyObject *lookup_dict) {
  list_T *l;
...
static int convert_dl(PyObject *obj, typval_T *tv, pytotvfunc py_to_tv, PyObject *lookup_dict) {
  PyObject *capsule;
...
static int ConvertFromPyMapping(PyObject *obj, typval_T *tv) {
  PyObject *lookup_dict;
  int ret;
  ...
  if (PyType_IsSubtype(obj->ob_type, &DictionaryType)) {
    ...
    PyErr_FORMAT(PyExc_TypeError, N_("unable to convert %s to vim dictionary"), Py_TYPE_NAME(obj));
    ret = -1;
  }
  Py_DECREF(lookup_dict);
  return ret;
...
static int ConvertFromPySequence(PyObject *obj, typval_T *tv) {
  PyObject *lookup_dict;
  int ret;
  ...
  if (PyType_IsSubtype(obj->ob_type, &ListType)) {
    ...
    PyErr_FORMAT(PyExc_TypeError, N_("unable to convert %s to vim list"), Py_TYPE_NAME(obj));
    ret = -1;
  }
  Py_DECREF(lookup_dict);
  return ret;
...
static int ConvertFromPyObject(PyObject *obj, typval_T *tv) {
  PyObject *lookup_dict;
  int ret;
...
static int _ConvertFromPyObject(PyObject *obj, typval_T *tv, PyObject *lookup_dict) {
  if (PyType_IsSubtype(obj->ob_type, &DictionaryType)) {
    tv->v_type = VAR_DICT;
    tv->vval.v_dict = (((DictionaryObject *)(obj))->dict);
    ++tv->vval.v_dict->dv_refcount;
  ...
  /* PyObject_GetIter can create built-in iterator for any sequence object */
    ...
    PyErr_FORMAT(PyExc_TypeError, N_("unable to convert %s to vim structure"), Py_TYPE_NAME(obj));
    return -1;
  }
  return 0;
}

static PyObject *ConvertToPyObject(typval_T *tv) {
  typval_T *argv;
  int i;
  if (tv == NULL) {
  ...
  switch (tv->v_type) {
  case VAR_STRING:
    return PyBytes_FromString(tv->vval.v_string == NULL ? "" : (char *)tv->vval.v_string);
  case VAR_NUMBER:
  ...
  case VAR_FUNC:
    return NEW_FUNCTION(tv->vval.v_string == NULL ? (char_u *)"" : tv->vval.v_string, 0, NULL, NULL,
                        TRUE);
  case VAR_PARTIAL:
    ...
    return NEW_FUNCTION(tv->vval.v_partial == NULL ? (char_u *)""
                                                   : partial_name(tv->vval.v_partial),
                        tv->vval.v_partial->pt_argc, argv, tv->vval.v_partial->pt_dict,
                        tv->vval.v_partial->pt_auto);
  case VAR_BLOB:
    return PyBytes_FromStringAndSize((char *)tv->vval.v_blob->bv_ga.ga_data,
...
static BufMapObject TheBufferMap = {PyObject_HEAD_INIT(&BufMapType)};

static WinListObject TheWindowList = {PyObject_HEAD_INIT(&WinListType) NULL};

static CurrentObject TheCurrent = {PyObject_HEAD_INIT(&CurrentType)};

static TabListObject TheTabPageList = {PyObject_HEAD_INIT(&TabListType)};
...
static int populate_module(PyObject *m) {
  int i;
  ...
  for (i = 0; i < (int)(sizeof(numeric_constants) / sizeof(struct numeric_constant)); ++i)
    ADD_CHECKED_OBJECT(m, numeric_constants[i].name, PyInt_FromLong(numeric_constants[i].val));

  for (i = 0; i < (int)(sizeof(object_constants) / sizeof(struct object_constant)); ++i) {
    PyObject *valObject;
  ...
  ADD_CHECKED_OBJECT(m, "options", OptionsNew(SREQ_GLOBAL, NULL, dummy_check, NULL));
