...
static Scheme_Object *(*dll_scheme_lookup_global)(Scheme_Object *symbol,
                                                  Scheme_Env *env);
static Scheme_Object *(*dll_scheme_make_closed_prim_w_arity)(Scheme_Closed_Prim *prim, void *data, const char *name, mzshort mina,
                                                             mzshort maxa);
...
void ex_mzfile(exarg_T *eap)
{
  ...
  if (do_mzscheme_command(eap, &pinfo, do_load) != OK && pinfo.port != NULL) /* looks like port was not closed */
  {
    scheme_close_input_port(pinfo.port);
    MZ_GC_CHECK();
  }
...
get_buffer_by_name(void *data, int argc, Scheme_Object **argv)
{
  ...
  FOR_ALL_BUFFERS(buf)
  {
    ...
    else if (!fnamecmp(buf->b_ffname, BYTE_STRING_VALUE(fname)) || !fnamecmp(buf->b_sfname, BYTE_STRING_VALUE(fname)))
    {
...
#endif
  else if (SCHEME_VECTORP(obj) || SCHEME_NULLP(obj) || SCHEME_PAIRP(obj) || SCHEME_MUTABLE_PAIRP(obj))
  {
...
sandbox_file_guard(int argc UNUSED, Scheme_Object **argv)
{
  if (sandbox)
  {
    ...
    while (!SCHEME_NULLP(requested_access))
    {
      ...
      if (scheme_eq(item, M_write) || scheme_eq(item, M_read) || scheme_eq(item, M_execute) || scheme_eq(item, M_delete))
        raise_vim_exn(_("not allowed in the Vim sandbox"));
