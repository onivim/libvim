 ...
 */
int msg(char *s) { return msg_attr_keep(s, 0, FALSE); }
...
int msg_attr(char *s, int attr) { return msg_attr_keep(s, attr, FALSE); }

int msg_attr_keep(char *s, int attr, int keep) /* TRUE: set keep_msg if it doesn't scroll */
{
   ...
   * truncated message) */
  if ((char_u *)s != keep_msg || (*s != '<' && last_msg_hist != NULL &&
                                  last_msg_hist->msg != NULL && STRCMP(s, last_msg_hist->msg)))
 ...
 */
void internal_error(char *where) { siemsg(_(e_intern2), where); }

/* emsg3() and emsgn() are in misc2.c to avoid warnings for the prototypes. */

void emsg_invreg(int name) { semsg(_("E354: Invalid register name: '%s'"), transchar(name)); }
...
static void add_msg_hist(char_u *s, int len, /* -1 for undetermined length */
                         int attr) {
  struct msg_hist *p;
  ...
  if (p == first_msg_hist) {
    ...
    if (s != NULL && *s != NUL)
      ...
      msg_attr(
          ...
          _("Messages maintainer: Bram Moolenaar <Bram@vim.org>"), HL_ATTR(HLF_T));
  }
...
void msg_putchar(int c) { msg_putchar_attr(c, 0); }
...
void msg_home_replace(char_u *fname) { msg_home_replace_attr(fname, 0); }

#if defined(FEAT_FIND_ID) || defined(PROTO)
void msg_home_replace_hl(char_u *fname) { msg_home_replace_attr(fname, HL_ATTR(HLF_D)); }
#endif
 ...
 */
int msg_outtrans(char_u *str) { return msg_outtrans_attr(str, 0); }

int msg_outtrans_attr(char_u *str, int attr) {
  return msg_outtrans_len_attr(str, (int)STRLEN(str), attr);
}

int msg_outtrans_len(char_u *str, int len) { return msg_outtrans_len_attr(str, len, 0); }
 ...
 */
int msg_outtrans_special(char_u *strstart,
 ...
 */
char_u *str2special_save(char_u *str, int is_lhs) /* TRUE for lhs, FALSE for rhs */
{
  garray_T ga;
  char_u *p = str;
 ...
 */
char_u *str2special(char_u **sp, int from) /* TRUE for lhs of mapping */
{
  int c;
  static char_u buf[7];
  char_u *str = *sp;
 ...
 */
void msg_puts(char *s) { msg_puts_attr(s, 0); }

void msg_puts_title(char *s) { msg_puts_attr(s, HL_ATTR(HLF_T)); }
 ...
 */
void msg_puts_attr(char *s, int attr) { msg_puts_attr_len(s, -1, attr); }
 ...
 */
void may_clear_sb_text(void) { do_clear_sb_text = SB_CLEAR_ALL; }
 ...
 */
void sb_text_end_cmdline(void) { do_clear_sb_text = SB_CLEAR_CMDLINE_DONE; }
 ...
 * different letter.
 */
int do_dialog(int type UNUSED, char_u *title UNUSED, char_u *message, char_u *buttons,
              int dfltbutton, char_u *textfield UNUSED, /* IObuff for inputdialog(), NULL
                                                           otherwise */
 ...
 */
static int copy_char(char_u *from, char_u *to, int lowercase) /* make character lower case */
{
  int len;
  int c;
 ...
 */
char_u *do_browse(int flags,       /* BROWSE_SAVE and BROWSE_DIR */
                  ...
                  buf_T *buf)      /* buffer to read/write for */
{
  ...
  if (initdir == NULL || *initdir == NUL) {
    ...
    if (STRCMP(p_bsdir, "last") != 0 && STRCMP(p_bsdir, "buffer") != 0 &&
        STRCMP(p_bsdir, "current") != 0 && mch_isdir(p_bsdir))
      initdir = p_bsdir;
  ...
  if (gui.in_use) /* when this changes, also adjust f_has()! */
  {
    ...
    if (flags & BROWSE_DIR) {
      ...
      fname = gui_mch_browse(flags & BROWSE_SAVE, title, dflt, ext, initdir, (char_u *)_(filter));
 ...
 */
static const char *infinity_str(int positive, char fmt_spec, int force_sign,
...
#define TMP_LEN 34
        ...
        case 'S':
          ...
          /* truncate string if necessary as requested by precision */
            ...
            char *q = memchr(str_arg, '\0',
                             precision <= (size_t)0x7fffffffL ? precision : (size_t)0x7fffffffL);
      ...
      case 'p': {
        ...
        if (fmt_spec == 'p') {
          ...
          ptr_arg =
#if defined(FEAT_EVAL)
              tvs != NULL ? (void *)tv_str(tvs, &arg_idx, NULL) :
#endif
                          va_arg(ap, void *);
          if (ptr_arg != NULL)
            arg_sign = 1;
          ...
          case 'h':
            uint_arg =
#if defined(FEAT_EVAL)
                tvs != NULL ? (unsigned)tv_nr(tvs, &arg_idx) :
#endif
          ...
          case 'l':
            ulong_arg =
#if defined(FEAT_EVAL)
                tvs != NULL ? (unsigned long)tv_nr(tvs, &arg_idx) :
#endif
        ...
        if (fmt_spec == 'd') {
          ...
          if (arg_sign != 0 &&
              (fmt_spec == 'b' || fmt_spec == 'B' || fmt_spec == 'x' || fmt_spec == 'X')) {
            tmp[str_arg_l++] = '0';
      ...
      case 'G': {
        ...
        if ((fmt_spec == 'f' || fmt_spec == 'F') &&
          ...
          if (remove_trailing_zeroes) {
             ...
             * case. */
            tp = (char *)vim_strchr((char_u *)tmp, fmt_spec == 'e' ? 'e' : 'E');
            if (tp != NULL && (tp[1] == '+' || tp[1] == '-') && tp[2] == '0' &&
                vim_isdigit(tp[3]) && vim_isdigit(tp[4])) {
              STRMOVE(tp + 2, tp + 3);
              --str_arg_l;
            }
          }
        ...
        if (pn > 0) {
          if (str_l < str_m) {
            ...
            vim_memset(str + str_l, zero_padding ? '0' : ' ',
                       (size_t)pn > avail ? avail : (size_t)pn);
          }
          str_l += pn;
        }
      }
       ...
       * (or as-is conversion specifier for unknown conversions) */
      {
        ...
        if (sn > 0) {
          if (str_l < str_m) {
            ...
            mch_memmove(str + str_l, str_arg + zero_padding_insertion_ind,
