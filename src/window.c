...
#define CHECK_CMDWIN \
  ...
  case 'L':
    ...
    win_totop((int)Prenum,
              ((nchar == 'H' || nchar == 'L') ? WSP_VERT : 0) | ((nchar == 'H' || nchar == 'K') ? WSP_TOP : WSP_BOT));
    break;
    ...
    int dir)
{
  ...
  if (flags & WSP_VERT)
  {
    ...
    if (flags & (WSP_TOP | WSP_BOT))
    {
      ...
      frame_new_width(curfrp, curfrp->fr_width - (new_size + ((flags & WSP_TOP) != 0)), flags & WSP_TOP,
                      FALSE);
    }
    else
...
void scroll_to_fraction(win_T *wp, int prev_height)
{
  ...
  //   is visible.
  if (height > 0 && (!wp->w_p_scb || wp == curwin) && (height < wp->w_buffer->b_ml.ml_line_count || wp->w_topline > 1))
  {
    /*
