...
#ifdef FEAT_RIGHTLEFT
  if (curwin->w_p_rl && *curwin->w_p_rlc == 's' && (context->firstc == '/' || context->firstc == '?'))
    cmdmsg_rl = TRUE;
  else
    cmdmsg_rl = FALSE;
#endif
...
executionStatus_T state_cmdline_execute(void *ctx, int c)
{
  ...
  /* free old command line when finished moving around in the history
	 * list */
  if (context->lookfor != NULL && c != K_S_DOWN && c != K_S_UP && c != K_DOWN && c != K_UP && c != K_PAGEDOWN && c != K_PAGEUP && c != K_KPAGEDOWN && c != K_KPAGEUP && c != K_LEFT && c != K_RIGHT && (context->xpc.xp_numfiles > 0 || (c != Ctrl_P && c != Ctrl_N)))
  ...
  /* Hitting CR after "emenu Name.": complete submenu */
  if (context->xpc.xp_context == EXPAND_MENUNAMES && p_wmnu && ccline.cmdpos > 1 && ccline.cmdbuff[ccline.cmdpos - 1] == '.' && ccline.cmdbuff[ccline.cmdpos - 2] != '\\' && (c == '\n' || c == '\r' || c == K_KENTER))
    c = K_DOWN;
#endif

  /* free expanded names when finished walking through matches */
  if (context->xpc.xp_numfiles != -1 && !(c == p_wc && KeyTyped) && c != p_wcm && c != Ctrl_N && c != Ctrl_P && c != Ctrl_A && c != Ctrl_L)
  {
  ...
  if ((context->xpc.xp_context == EXPAND_FILES || context->xpc.xp_context == EXPAND_DIRECTORIES || context->xpc.xp_context == EXPAND_SHELLCMD) && p_wmnu)
  {
    char_u upseg[5];
    ...
    else if (STRNCMP(context->xpc.xp_pattern, upseg + 1, 3) == 0 && c == K_DOWN)
    {
      ...
      if (found && ccline.cmdbuff[context->j - 1] == '.' && ccline.cmdbuff[context->j - 2] == '.' && (vim_ispathsep(ccline.cmdbuff[context->j - 3]) || context->j == context->i + 2))
      {
  ...
  case Ctrl_W:
    ...
    else if (ccline.cmdlen == 0 && c != Ctrl_W && ccline.cmdprompt == NULL && context->indent == 0)
    {
      /* In ex and debug mode it doesn't make sense to return. */
      if (exmode_active
#ifdef FEAT_EVAL
  ...
  case K_KPAGEDOWN:
    ...
    for (;;)
    {
      ...
      if ((c != K_UP && c != K_DOWN) || context->hiscnt == context->i || STRNCMP(history[context->histype][context->hiscnt].hisstr, context->lookfor, (size_t)context->j) == 0)
        break;
    }
    ...
    if (context->hiscnt != context->i) /* jumped to other entry */
    {
      ...
      if (context->histype == HIST_SEARCH && p != context->lookfor && (old_firstc = p[STRLEN(p) + 1]) != context->firstc)
      {
        /* Correct for the separator character used when
			 * adding the history entry vs the one used now.
			 * First loop: count length.
        ...
        for (context->i = 0; context->i <= 1; ++context->i)
        {
          ...
          for (context->j = 0; p[context->j] != NUL; ++context->j)
          {
            ...
            else
            {
              /* Escape new sep, unless it is already
				     * escaped. */
              if (p[context->j] == context->firstc && (context->j == 0 || p[context->j - 1] != '\\'))
              {
                if (context->i > 0)
                  ccline.cmdbuff[len] = '\\';
                ++len;
  ...
  if (context->do_abbr && (IS_SPECIAL(c) || !vim_iswordc(c)) && (ccheck_abbr(
                                                                     // Add ABBR_OFF for characters above 0x100, this is
                                                                     // what check_abbr() expects.
                                                                     (has_mbyte && c >= 0x100) ? (c + ABBR_OFF) : c) ||
                                                                 c == Ctrl_RSB))
    ...
    int writing) /* ignore entries read from viminfo */
{
  ...
  do
  {
    ...
    if (STRCMP(str, p) == 0 && !(writing && history[type][i].viminfo) && (type != HIST_SEARCH || sep == p[STRLEN(p) + 1]))
    {
      if (!move_to_front)
        return TRUE;
      last_i = i;
    ...
    int writing)
{
  ...
  if (values->ga_len < 4 || vp[0].bv_type != BVAL_NR || vp[1].bv_type != BVAL_NR || (vp[2].bv_type != BVAL_NR && vp[2].bv_type != BVAL_EMPTY) || vp[3].bv_type != BVAL_STRING)
    return;
...
void write_viminfo_history(FILE *fp, int merge)
{
  ...
  for (type = 0; type < HIST_COUNT; ++type)
  {
    ...
    fprintf(fp, _("\n# %s History (newest to oldest):\n"),
            type == HIST_CMD ? _("Command Line") : type == HIST_SEARCH ? _("Search String") : type == HIST_EXPR ? _("Expression") : type == HIST_INPUT ? _("Input Line") : _("Debug Line"));
    if (num_saved > hislen)
      num_saved = hislen;
