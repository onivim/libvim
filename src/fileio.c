...
retry:
  ...
  while (!error && !got_int)
  {
    ...
    else
    {
      ...
      if ((filesize == 0
#ifdef FEAT_CRYPT
           || (cryptkey != NULL && filesize == crypt_get_header_len(
                                                   crypt_get_method_nr(curbuf)))
#endif
               ) &&
          (fio_flags == FIO_UCSBOM || (!curbuf->b_p_bomb && tmpname == NULL && (*fenc == 'u' || (*fenc == NUL && enc_utf8)))))
      {
        char_u *ccname;
        int blen;
...
failed:
  ...
  if (!recoverymode)
  {
    ...
    if (newfile && (error || conv_error != 0 || (illegal_byte > 0 && bad_char_behavior != BAD_KEEP)))
      curbuf->b_p_ro = TRUE;
...
{
  return (STRNCMP(fname, "/dev/fd/", 8) == 0 && VIM_ISDIGIT(fname[8]) && *skipdigits(fname + 9) == NUL && (fname[9] != NUL || (fname[8] != '0' && fname[8] != '1' && fname[8] != '2')));
}
#endif
    ...
    int filtering)
{
  ...
  if (buf->b_ffname == NULL && reset_changed && whole && buf == curbuf
      ...
      && !bt_nofile(buf)
#endif
      && !filtering && (!append || vim_strchr(p_cpo, CPO_FNAMEAPP) != NULL) && vim_strchr(p_cpo, CPO_FNAMEW) != NULL)
  {
    if (set_rw_fname(fname, sfname) == FAIL)
      return FAIL;
  ...
  if (!(append && *p_pm == NUL) && !filtering && perm >= 0 && dobackup)
  {
    ...
    else if ((bkc & BKC_AUTO)) /* "auto" */
    {
      ...
      /*
	     ...
	     */
      if (st_old.st_nlink > 1 || mch_lstat((char *)fname, &st) < 0 || st.st_dev != st_old.st_dev || st.st_ino != st_old.st_ino
#ifndef HAVE_FCHOWN
...
#define TRUNC_ON_OPEN O_TRUNC
  ...
  if (!checking_conversion)
  {
    ...
    if (backup != NULL && !backup_copy)
    {
      ...
      if (mch_stat((char *)wfname, &st) < 0 || st.st_uid != st_old.st_uid || st.st_gid != st_old.st_gid)
      {
  ...
  if (*p_pm && dobackup)
  {
    ...
    else
    {
      ...
      if (org == NULL || (empty_fd = mch_open(org,
    ...
    int flags)
{
  ...
  if (p[0] == 0xef && p[1] == 0xbb && size >= 3 && p[2] == 0xbf && (flags == FIO_ALL || flags == FIO_UTF8 || flags == 0))
  {
    name = "utf-8"; /* EF BB BF */
    len = 3;
  }
  ...
  {
    if (size >= 4 && p[2] == 0 && p[3] == 0 && (flags == FIO_ALL || flags == (FIO_UCS4 | FIO_ENDIAN_L)))
    {
  ...
  else if (p[0] == 0xfe && p[1] == 0xff && (flags == FIO_ALL || flags == FIO_UCS2 || flags == FIO_UTF16))
  {
  ...
  else if (size >= 4 && p[0] == 0 && p[1] == 0 && p[2] == 0xfe && p[3] == 0xff && (flags == FIO_ALL || flags == FIO_UCS4))
  {
...
shorten_fname(char_u *full_path, char_u *dir_name)
{
  ...
  /*
     ...
     */
  else if (len > 3 && TOUPPER_LOC(full_path[0]) == TOUPPER_LOC(dir_name[0]) && full_path[1] == ':' && vim_ispathsep(full_path[2]))
    p = full_path + 2;
#endif
  else
    p = NULL;
...
void shorten_buf_fname(buf_T *buf, char_u *dirname, int force)
{
  ...
  if (buf->b_fname != NULL
      ...
      && !bt_nofile(buf)
#endif
      && !path_with_url(buf->b_fname) && (force || buf->b_sfname == NULL || mch_isFullName(buf->b_sfname)))
  {
...
int vim_rename(char_u *from, char_u *to)
{
...
#ifdef MSWIN
  {
    ...
    if (win32_fileinfo(from, &info1) == FILEINFO_OK && win32_fileinfo(to, &info2) == FILEINFO_OK && info1.dwVolumeSerialNumber == info2.dwVolumeSerialNumber && info1.nFileIndexHigh == info2.nFileIndexHigh && info1.nFileIndexLow == info2.nFileIndexLow)
      use_tmp_file = TRUE;
  }
#endif
