...
void scroll_cursor_bot(int min_scroll, int set_topbot)
{
  ...
  while (loff.lnum > 1)
  {
    ...
    if ((((scrolled <= 0 || scrolled >= min_scroll) && extra >= (so)) || boff.lnum + 1 > curbuf->b_ml.ml_line_count) && loff.lnum <= curwin->w_botline
#ifdef FEAT_DIFF
...
int onepage(int dir, long count)
{
  ...
  for (; count > 0; --count)
  {
	 ...
	 */
    if (dir == FORWARD
            ? ((curwin->w_topline >= curbuf->b_ml.ml_line_count - so) && curwin->w_botline > curbuf->b_ml.ml_line_count)
            : (curwin->w_topline == 1
#ifdef FEAT_DIFF
               && curwin->w_topfill ==
