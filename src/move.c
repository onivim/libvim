 ...
 */
void changed_window_setting(void) { changed_window_setting_win(curwin); }
 ...
 */
void validate_virtcol(void) { validate_virtcol_win(curwin); }
...
int curwin_col_off(void) { return win_col_off(curwin); }
...
int curwin_col_off2(void) { return win_col_off2(curwin); }
...
void curs_columns(int may_scroll) /* when TRUE, may scroll horizontally */
{
  ...
  if ((curwin->w_wrow >= curwin->w_height ||
       ((prev_skipcol > 0 || curwin->w_wrow + so >= curwin->w_height) &&
        (p_lines =
#ifdef FEAT_DIFF
             plines_win_nofill
#else
...
void scroll_cursor_bot(int min_scroll, int set_topbot) {
  ...
  while (loff.lnum > 1) {
    ...
    if ((((scrolled <= 0 || scrolled >= min_scroll) && extra >= (so)) ||
    ...
    if (boff.lnum < curbuf->b_ml.ml_line_count) {
      ...
      if (used > curwin->w_height)
        break;
      if (extra < (so) || scrolled < min_scroll) {
        extra += boff.height;
