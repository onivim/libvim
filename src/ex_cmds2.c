...
long proftime_time_left(proftime_T *due, proftime_T *now) {
  ...
  if (now->QuadPart > due->QuadPart)
    return 0;
  QueryPerformanceFrequency(&fr);
  return (long)(((double)(due->QuadPart - now->QuadPart) / (double)fr.QuadPart) * 1000);
#else
...
static void timer_callback(timer_T *timer) {
  ...
  call_callback(&timer->tr_callback, -1, &rettv, 1, argv, NULL, 0L, 0L, &dummy, TRUE, NULL);
  clear_tv(&rettv);
}
...
void add_timer_info(typval_T *rettv, timer_T *timer) {
  ...
  dict_add_number(dict, "repeat", (long)(timer->tr_repeat < 0 ? -1 : timer->tr_repeat + 1));
...
void profile_self(proftime_T *self, proftime_T *total, proftime_T *children) {
  ...
  if (total->tv_sec < children->tv_sec ||
      (total->tv_sec == children->tv_sec && total->tv_usec <= children->tv_usec))
    return;
#endif
 ...
 */
void profile_get_wait(proftime_T *tm) { *tm = prof_wait_time; }
...
static char *pexpand_cmds[] = {"start",
 ...
 * specific expansion.
 */
char_u *get_profile_name(expand_T *xp UNUSED, int idx) {
  switch (pexpand_what) {
  case PEXP_SUBCMD:
 ...
 */
void prof_inchar_enter(void) { profile_start(&inchar_time); }
...
void browse_save_fname(buf_T *buf) {
  if (buf->b_fname == NULL) {
    ...
    fname = do_browse(BROWSE_SAVE, (char_u *)_("Save As"), NULL, NULL, NULL, NULL, buf);
    if (fname != NULL) {
 ...
 */
void dialog_changed(buf_T *buf, int checkall) /* may abandon all changed buffers */
{
 ...
 */
int check_changed_any(int hidden, /* Only check hidden buffers */
                      int unload) {
  int ret = FALSE;
  buf_T *buf;
  int save;
 ...
 * growarray "gap".
 */
static int get_arglist(garray_T *gap, char_u *str, int escaped) {
...
int get_arglist_exp(char_u *str, int *fcountp, char_u ***fnamesp, int wig) {
  ...
  if (get_arglist(&ga, str, TRUE) == FAIL)
    return FAIL;
  if (wig == TRUE)
    i = expand_wildcards(ga.ga_len, (char_u **)ga.ga_data, fcountp, fnamesp, EW_FILE | EW_NOTFOUND);
  else
 ...
 */
void set_arglist(char_u *str) { do_arglist(str, AL_SET, 0, FALSE); }
 ...
 * Return FAIL for failure, OK otherwise.
 */
static int do_arglist(char_u *str, int what,
                      ...
                      int will_edit)    // will edit added argument
{
  ...
  if (what == AL_DEL) {
    ...
    i = expand_wildcards(new_ga.ga_len, (char_u **)new_ga.ga_data, &exp_count, &exp_files,
...
static int editing_arg_idx(win_T *win) {
  return !(
      ...
      (win->w_buffer->b_fnum != WARGLIST(win)[win->w_arg_idx].ae_fnum &&
       (win->w_buffer->b_ffname == NULL || !(fullpathcmp(alist_name(&WARGLIST(win)[win->w_arg_idx]),
...
void check_arg_idx(win_T *win) {
  if (WARGCOUNT(win) > 1 && !editing_arg_idx(win)) {
    ...
    if (win->w_arg_idx != WARGCOUNT(win) - 1 && arg_had_last == FALSE &&
        ALIST(win) == &global_alist && GARGCOUNT > 0 && win->w_arg_idx < GARGCOUNT &&
        (win->w_buffer->b_fnum == GARGLIST[GARGCOUNT - 1].ae_fnum ||
         (win->w_buffer->b_ffname != NULL &&
          (fullpathcmp(alist_name(&GARGLIST[GARGCOUNT - 1]), win->w_buffer->b_ffname, TRUE, TRUE) &
           FPC_SAME))))
      arg_had_last = TRUE;
  } else {
 ...
 */
void ex_rewind(exarg_T *eap) { do_argfile(eap, 0); }

/*
 * ":last" and ":slast".
 */
void ex_last(exarg_T *eap) { do_argfile(eap, ARGCOUNT - 1); }
 ...
 */
static void alist_add_list(int count, char_u **files,
 ...
 */
char_u *get_arglist_name(expand_T *xp UNUSED, int idx) {
  if (idx >= ARGCOUNT)
    return NULL;
...
static void source_callback(char_u *fname, void *cookie UNUSED) {
 ...
 */
int do_in_path(char_u *path, char_u *name, int flags, void (*callback)(char_u *fname, void *ck),
               void *cookie) {
  char_u *rtp;
  char_u *np;
  char_u *buf;
 ...
 * has done its job.
 */
static int do_in_path_and_pp(char_u *path, char_u *name, int flags,
 ...
 */
int do_in_runtimepath(char_u *name, int flags, void (*callback)(char_u *fname, void *ck),
                      void *cookie) {
 ...
 * return FAIL when no file could be sourced, OK otherwise.
 */
int source_runtime(char_u *name, int flags) { return source_in_path(p_rtp, name, flags); }
...
static int add_pack_dir_to_rtp(char_u *fname) {
  ...
  *p4 = c;
  ...
  if (afterlen > 0 && after_insp != NULL) {
    ...
    mch_memmove(new_rtp + new_rtp_len, p_rtp + keep, keep_after - keep);
...
void add_pack_start_dirs(void) {
  do_in_path(p_pp, (char_u *)"pack/*/start/*", DIP_ALL + DIP_DIR, add_pack_plugin, &APP_ADD_DIR);
}
...
void load_start_packages(void) {
  ...
  do_in_path(p_pp, (char_u *)"pack/*/start/*", DIP_ALL + DIP_DIR, add_pack_plugin, &APP_LOAD);
}
...
void ex_packadd(exarg_T *eap) {
  ...
  for (round = 1; round <= 2; ++round) {
    ...
    res = do_in_path(p_pp, (char_u *)pat,
                     DIP_ALL + DIP_DIR + (round == 2 && res == FAIL ? DIP_ERR : 0), add_pack_plugin,
 ...
 * ":options"
 */
void ex_options(exarg_T *eap UNUSED) {
  vim_setenv((char_u *)"OPTWIN_CMD",
             (char_u *)(cmdmod.tab ? "tab" : (cmdmod.split & WSP_VERT) ? "vert" : ""));
...
static void source_pyx_file(exarg_T *eap, char_u *fname) {
  ...
  if (v == 2) {
    ...
    vim_snprintf((char *)IObuff, IOSIZE,
                 _("W20: Required python version 2.x not supported, ignoring file: %s"), fname);
    msg((char *)IObuff);
#endif
    return;
  } else {
...
#else
    vim_snprintf((char *)IObuff, IOSIZE,
                 _("W21: Required python version 3.x not supported, ignoring file: %s"), fname);
    msg((char *)IObuff);
#endif
    return;
  }
 ...
 * ":pyxfile {fname}"
 */
void ex_pyxfile(exarg_T *eap) { source_pyx_file(eap, eap->arg); }
...
void ex_source(exarg_T *eap) {
  ...
  if (cmdmod.browse) {
    ...
    fname = do_browse(0, (char_u *)_("Source Vim script"), eap->arg, NULL, NULL,
 ...
 */
linenr_T *source_breakpoint(void *cookie) { return &((struct source_cookie *)cookie)->breakpoint; }

/*
 * Return the address holding the debug tick for a source cookie.
 */
int *source_dbg_tick(void *cookie) { return &((struct source_cookie *)cookie)->dbg_tick; }

/*
 * Return the nesting level for a source cookie.
 */
int source_level(void *cookie) { return ((struct source_cookie *)cookie)->level; }
#endif
 ...
 */
int do_source(char_u *fname, int check_other, /* check for .vimrc and _vimrc */
              int is_vimrc)                   /* DOSO_ value */
{
  ...
  if (cookie.fp == NULL && check_other) {
    ...
    p = gettail(fname_exp);
    if ((*p == '.' || *p == '_') &&
        (STRICMP(p + 1, "vimrc") == 0 || STRICMP(p + 1, "gvimrc") == 0 ||
         STRICMP(p + 1, "exrc") == 0)) {
      if (*p == '_')
        *p = '.';
      else
...
#endif
  for (current_sctx.sc_sid = script_items.ga_len; current_sctx.sc_sid > 0; --current_sctx.sc_sid) {
    si = &SCRIPT_ITEM(current_sctx.sc_sid);
    if (si->sn_name != NULL && (
#ifdef UNIX
  ...
  if (current_sctx.sc_sid == 0) {
    ...
    if (ga_grow(&script_items, (int)(current_sctx.sc_sid - script_items.ga_len)) == FAIL)
      goto almosttheend;
 ...
 */
char_u *getsourceline(int c UNUSED, void *cookie, int indent UNUSED) {
  ...
  if (line != NULL && (vim_strchr(p_cpo, CPO_CONCAT) == NULL)) {
    ...
    if (sp->nextline != NULL &&
        (*(p = skipwhite(sp->nextline)) == '\\' || (p[0] == '"' && p[1] == '\\' && p[2] == ' '))) {
      garray_T ga;
 ...
 */
void ex_scriptversion(exarg_T *eap UNUSED) {
#ifdef FEAT_EVAL
  int nr;
 ...
 * supported languages.
 */
static char_u *gettext_lang(char_u *name) {
  int i;
  static char *(mtable[]) = {"afrikaans",
                             "af",
                             "czech",
                             "cs",
                             "dutch",
 ...
 */
static int is_valid_mess_lang(char_u *lang) {
...
static char_u **find_locales(void) {
  ...
  char_u *locale_a = get_cmd_output((char_u *)"locale -a", NULL, SHELL_SILENT, NULL);
  if (locale_a == NULL)
    return NULL;
 ...
 * ":language" command.
 */
char_u *get_lang_arg(expand_T *xp UNUSED, int idx) {
  if (idx == 0)
 ...
 */
char_u *get_locales(expand_T *xp UNUSED, int idx) {
  init_locales();
  if (locales == NULL)
    return NULL;
  return locales[idx];
