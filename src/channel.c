...
channel_still_useful(channel_T *channel)
{
  ...
  return (channel->ch_callback.cb_name != NULL && (has_sock_msg || has_out_msg || has_err_msg)) || ((channel->ch_part[PART_OUT].ch_callback.cb_name != NULL || channel->ch_part[PART_OUT].ch_bufref.br_buf != NULL) && has_out_msg) || ((channel->ch_part[PART_ERR].ch_callback.cb_name != NULL || channel->ch_part[PART_ERR].ch_bufref.br_buf != NULL) && has_err_msg);
}
  ...
  *p = NUL;
  ...
  while (p < res + len)
  {
    ...
    else if (*p == 0x1b)
    {
      ...
      if (p + 3 < res + len && p[1] == ']' && (p[2] == '0' || p[2] == '1' || p[2] == '2') && p[3] == ';')
      {
        // '\a' becomes a NL
...
append_to_buffer(buf_T *buffer, char_u *msg, channel_T *channel, ch_part_T part)
{
  ...
  if (buffer->b_nwindows > 0)
  {
    ...
    FOR_ALL_WINDOWS(wp)
    {
      if (wp->w_buffer == buffer && (save_write_to
    ...
    if (res >= 0 && ch_part->ch_nonblocking)
    {
      ...
      else
      {
        ...
        if (wq->wq_prev != NULL && wq->wq_prev->wq_ga.ga_len + len < 4000)
        {
          ...
          {
            mch_memmove((char *)last->wq_ga.ga_data + last->wq_ga.ga_len,
                        buf, len);
...
{
  return job->jv_status == JOB_STARTED && (job->jv_stoponexit != NULL || job->jv_exit_cb.cb_name != NULL);
}
...
int has_pending_job(void)
{
  ...
  for (job = first_job; job != NULL; job = job->jv_next)
    ...
    if ((job->jv_status == JOB_STARTED && !job_channel_still_useful(job)) || (job->jv_status == JOB_FINISHED && job_channel_can_close(job)))
      return TRUE;
  return FALSE;
}
...
#define USE_ARGV
  ...
  else
  {
    ...
    opt.jo_mode = MODE_NL;
    if (get_job_options(&argvars[1], &opt,
                        JO_MODE_ALL + JO_CB_ALL + JO_TIMEOUT_ALL + JO_STOPONEXIT + JO_EXIT_CB + JO_OUT_IO + JO_BLOCK_WRITE,
