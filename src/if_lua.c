...
luaV_buffer_index(lua_State *L)
{
  ...
  else if (lua_isstring(L, 2))
  {
    ...
    /* methods */
    else if (strncmp(s, "insert", 6) == 0 || strncmp(s, "next", 4) == 0 || strncmp(s, "previous", 8) == 0 || strncmp(s, "isvalid", 7) == 0)
    {
      lua_getmetatable(L, 1);
      lua_getfield(L, -1, s);
    }
...
luaV_window_index(lua_State *L)
{
  ...
  /* methods */
  else if (strncmp(s, "next", 4) == 0 || strncmp(s, "previous", 8) == 0 || strncmp(s, "isvalid", 7) == 0)
  {
    lua_getmetatable(L, 1);
    lua_getfield(L, -1, s);
  }
  ...
  if (lua_isstring(L, 1)) /* get by number or name? */
  {
    ...
    else // by name
    {
      ...
      FOR_ALL_BUFFERS(buf)
      {
        ...
        else if (strncmp(s, (char *)buf->b_ffname, l) == 0 || strncmp(s, (char *)buf->b_sfname, l) == 0)
          break;
      }
    }
  }
