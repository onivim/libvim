...
static void *luaV_checkcache(lua_State *L, void *p) {
  luaV_getudata(L, p);
  if (lua_isnil(L, -1))
 ...
 * Returns OK or FAIL.
 */
static int luaV_totypval(lua_State *L, int pos, typval_T *tv) {
  int status = OK;
...
static luaV_List *luaV_newlist(lua_State *L, list_T *lis) {
...
luaV_pushtype(list_T, list, luaV_List) luaV_type_tostring(list, LUAVIM_LIST)

    static int luaV_list_len(lua_State *L) {
  list_T *l = luaV_unbox(L, luaV_List, 1);
  lua_pushinteger(L, (int)list_len(l));
  return 1;
}

static int luaV_list_iter(lua_State *L) {
...
static int luaV_list_call(lua_State *L) {
  list_T *l = luaV_unbox(L, luaV_List, 1);
...
static int luaV_list_index(lua_State *L) {
  list_T *l = luaV_unbox(L, luaV_List, 1);
  ...
  } else if (lua_isstring(L, 2)) /* method? */
  {
    ...
    if (strncmp(s, "add", 3) == 0 || strncmp(s, "insert", 6) == 0) {
...
static int luaV_list_newindex(lua_State *L) {
  list_T *l = luaV_unbox(L, luaV_List, 1);
...
static int luaV_list_add(lua_State *L) {
...
static int luaV_list_insert(lua_State *L) {
  luaV_List *lis = luaV_checkudata(L, 1, LUAVIM_LIST);
  list_T *l = (list_T *)luaV_checkcache(L, (void *)*lis);
...
static const luaL_Reg luaV_List_mt[] = {
    {"__tostring", luaV_list_tostring}, {"__len", luaV_list_len},
    {"__call", luaV_list_call},         {"__index", luaV_list_index},
    {"__newindex", luaV_list_newindex}, {"add", luaV_list_add},
    {"insert", luaV_list_insert},       {NULL, NULL}};

/* =======   Dict type   ======= */

static luaV_Dict *luaV_newdict(lua_State *L, dict_T *dic) {
...
luaV_pushtype(dict_T, dict, luaV_Dict) luaV_type_tostring(dict, LUAVIM_DICT)

    static int luaV_dict_len(lua_State *L) {
  dict_T *d = luaV_unbox(L, luaV_Dict, 1);
  lua_pushinteger(L, (int)dict_len(d));
  return 1;
}

static int luaV_dict_iter(lua_State *L UNUSED) {
#ifdef FEAT_EVAL
...
static int luaV_dict_call(lua_State *L) {
  dict_T *d = luaV_unbox(L, luaV_Dict, 1);
...
static int luaV_dict_index(lua_State *L) {
  dict_T *d = luaV_unbox(L, luaV_Dict, 1);
...
static int luaV_dict_newindex(lua_State *L) {
  dict_T *d = luaV_unbox(L, luaV_Dict, 1);
  char_u *key = (char_u *)luaL_checkstring(L, 2);
  dictitem_T *di;
  typval_T v;
...
static const luaL_Reg luaV_Dict_mt[] = {
    {"__tostring", luaV_dict_tostring}, {"__len", luaV_dict_len},
    {"__call", luaV_dict_call},         {"__index", luaV_dict_index},
    {"__newindex", luaV_dict_newindex}, {NULL, NULL}};

/* =======   Blob type   ======= */

static luaV_Blob *luaV_newblob(lua_State *L, blob_T *blo) {
...
luaV_pushtype(blob_T, blob, luaV_Blob) luaV_type_tostring(blob, LUAVIM_BLOB)

    static int luaV_blob_gc(lua_State *L) {
  blob_T *b = luaV_unbox(L, luaV_Blob, 1);
  blob_unref(b);
  return 0;
}

static int luaV_blob_len(lua_State *L) {
  blob_T *b = luaV_unbox(L, luaV_Blob, 1);
  lua_pushinteger(L, (int)blob_len(b));
  return 1;
}

static int luaV_blob_index(lua_State *L) {
  blob_T *b = luaV_unbox(L, luaV_Blob, 1);
  if (lua_isnumber(L, 2)) {
...
static int luaV_blob_newindex(lua_State *L) {
  blob_T *b = luaV_unbox(L, luaV_Blob, 1);
  if (b->bv_lock)
...
static int luaV_blob_add(lua_State *L) {
...
static const luaL_Reg luaV_Blob_mt[] = {{"__tostring", luaV_blob_tostring},
                                        {"__gc", luaV_blob_gc},
                                        {"__len", luaV_blob_len},
...
static luaV_Funcref *luaV_newfuncref(lua_State *L, char_u *name) {
...
luaV_type_tostring(funcref, LUAVIM_FUNCREF)
    ...
    static int luaV_funcref_gc(lua_State *L) {
  luaV_Funcref *f = (luaV_Funcref *)lua_touserdata(L, 1);
...
static int luaV_funcref_len(lua_State *L) {
  luaV_Funcref *f = (luaV_Funcref *)lua_touserdata(L, 1);
...
static int luaV_funcref_call(lua_State *L) {
  luaV_Funcref *f = (luaV_Funcref *)lua_touserdata(L, 1);
...
static int luaV_buffer_index(lua_State *L) {
  ...
  if (n > 0 && n <= b->b_ml.ml_line_count)
    ...
    if (strncmp(s, "name", 4) == 0)
      lua_pushstring(L, (b->b_sfname == NULL) ? "" : (char *)b->b_sfname);
    else if (strncmp(s, "fname", 5) == 0)
      lua_pushstring(L, (b->b_ffname == NULL) ? "" : (char *)b->b_ffname);
    else if (strncmp(s, "number", 6) == 0)
      lua_pushinteger(L, b->b_fnum);
    /* methods */
    else if (strncmp(s, "insert", 6) == 0 || strncmp(s, "next", 4) == 0 ||
             strncmp(s, "previous", 8) == 0 || strncmp(s, "isvalid", 7) == 0) {
      lua_getmetatable(L, 1);
      lua_getfield(L, -1, s);
    } else
      lua_pushnil(L);
...
static const luaL_Reg luaV_Buffer_mt[] = {
    {"__tostring", luaV_buffer_tostring}, {"__len", luaV_buffer_len},
    {"__call", luaV_buffer_call},         {"__index", luaV_buffer_index},
    {"__newindex", luaV_buffer_newindex}, {"insert", luaV_buffer_insert},
    {"next", luaV_buffer_next},           {"previous", luaV_buffer_previous},
    {"isvalid", luaV_buffer_isvalid},     {NULL, NULL}};
...
static int luaV_window_index(lua_State *L) {
  ...
  /* methods */
  else if (strncmp(s, "next", 4) == 0 || strncmp(s, "previous", 8) == 0 ||
...
static const luaL_Reg luaV_Window_mt[] = {
    {"__tostring", luaV_window_tostring}, {"__call", luaV_window_call},
    {"__index", luaV_window_index},       {"__newindex", luaV_window_newindex},
    {"next", luaV_window_next},           {"previous", luaV_window_previous},
    {"isvalid", luaV_window_isvalid},     {NULL, NULL}};
...
static int luaV_debug(lua_State *L) {
  lua_settop(L, 0);
  lua_getglobal(L, "vim");
...
static int luaV_beep(lua_State *L UNUSED) {
  vim_beep(BO_LANG);
  return 0;
}
...
static int luaV_list(lua_State *L) {
  list_T *l;
...
static int luaV_dict(lua_State *L) {
  dict_T *d;
  int initarg = !lua_isnoneornil(L, 1);
...
static int luaV_blob(lua_State *L) {
  blob_T *b;
  int initarg = !lua_isnoneornil(L, 1);
...
static int luaV_funcref(lua_State *L) {
...
#endif
  if (lua_isstring(L, 1))
    s = (char_u *)lua_tostring(L, 1);
  luaV_pushbuffer(L, buflist_new(s, NULL, 1L, BLN_LISTED));
  return 1;
}

static int luaV_type(lua_State *L) {
  luaL_checkany(L, 1);
...
static const luaL_Reg luaV_module[] = {{"command", luaV_command},
                                       {"eval", luaV_eval},
                                       {"beep", luaV_beep},
                                       {"line", luaV_line},
                                       {"list", luaV_list},
...
static int luaV_free(lua_State *L) {
  lua_pushnil(L);
  luaV_setudata(L, lua_touserdata(L, 1));
  return 0;
}

static int luaV_luaeval(lua_State *L) {
  luaL_Buffer b;
  size_t l;
...
static int luaV_setref(lua_State *L) {
...
static int lua_isopen(void) { return L != NULL; }
