    ...
    typval_T *dict_arg)
{
  ...
  for (lnum = start_lnum; lnum <= end_lnum; ++lnum)
  {
    ...
    tmp_prop.tp_flags = (lnum > start_lnum ? TP_FLAG_CONT_PREV : 0) | (lnum < end_lnum ? TP_FLAG_CONT_NEXT : 0);
    ...
    int flags)
{
  ...
  for (ri = 0; ri < proplen; ++ri)
  {
    ...
    else if (tmp_prop.tp_len > 0 && tmp_prop.tp_col + tmp_prop.tp_len > col + ((pt != NULL && (pt->pt_flags & PT_FLAG_INS_END_INCL))
                                                                                   ? 0
                                                                                   : 1))
    {
