 ...
 */
int clip_mch_own_selection(VimClipboard *cbd UNUSED) {
  /*
 ...
 */
static char_u *crnl_to_nl(const char_u *str, int *size) {
  int pos = 0;
  int str_len = *size;
  char_u *ret;
  char_u *retp;
 ...
 */
static int vim_open_clipboard(void) {
  int delay = 10;
...
void clip_mch_set_selection(VimClipboard *cbd) {
  ...
  /* Set up metadata: */
  {
    ...
    hMemVim = GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, sizeof(VimClipType_t));
...
char_u *utf16_to_enc(short_u *str, int *lenp) {
  ...
  if (utf8_str != NULL) {
    *lenp = utf16_to_utf8(str, *lenp, utf8_str);
    ...
    conv.vc_type = CONV_NONE;
    convert_setup(&conv, (char_u *)"utf-8", p_enc ? p_enc : (char_u *)"latin1");
    if (conv.vc_type == CONV_NONE) {
