 ...
 * in check_termcode() */
    ...
    /*
     * These codes are valid for the Win32 Console .  The entries that start
     * with ESC | are translated into console calls in os_win32.c.  The function
     * keys are also translated in os_win32.c.
     */
    {(int)KS_NAME, "win32"},
...
#endif
#ifdef TERMINFO
    {(int)KS_CS, IF_EB("\033[%i%p1%d;%p2%dr", ESC_STR "[%i%p1%d;%p2%dr")},
#else
    ...
    {(int)KS_CEC, "\007"},
#ifdef TERMINFO
    {(int)KS_CWS, IF_EB("\033[8;%p1%d;%p2%dt", ESC_STR "[8;%p1%d;%p2%dt")},
    {(int)KS_CWP, IF_EB("\033[3;%p1%d;%p2%dt", ESC_STR "[3;%p1%d;%p2%dt")},
...
static void report_term_error(char *error_msg, char_u *term) {
...
static void report_default_term(char_u *term) {
 ...
 */
static char *tgetent_error(char_u *tbuf, char_u *term) {
  int i;
 ...
 */
void getlinecol(long *cp, /* pointer to columns */
 ...
 */
void term_windgoto(int row, int col) { OUT_STR(tgoto((char *)T_CM, col, row)); }
 ...
 */
void shell_resized(void) { set_shellsize(0, 0, FALSE); }
 ...
 */
int swapping_screen(void) { return (full_screen && *T_TI != NUL); }
    ...
    if (do_special && (do_lt || STRNCMP(src, "<lt>", 4) != 0)) {
      ...
      if (STRNICMP(src, "<SID>", 5) == 0) {
        if (current_sctx.sc_sid <= 0)
          ...
          sprintf((char *)result + dlen, "%ld", (long)current_sctx.sc_sid);
...
#ifdef VIMDLL
static guicolor_T gui_adjust_rgb(guicolor_T c) {
  if (gui.in_use)
    return c;
  else
