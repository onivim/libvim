 ...
 */
int is_not_a_term() { return params.not_a_term; }
 ...
 */
void main_loop(int cmdwin,   /* TRUE when working in the command-line window */
 ...
 */
static void early_arg_scan(mparm_T *parmp UNUSED) {
 ...
 */
static int get_number_arg(char_u *p, /* pointer to argument */
...
static void parse_command_name(mparm_T *parmp) {
  ...
  if (TOLOWER_ASC(initstr[0]) == 'e' &&
      (TOLOWER_ASC(initstr[1]) == 'v' || TOLOWER_ASC(initstr[1]) == 'g')) {
#ifdef FEAT_GUI
    gui.starting = TRUE;
#endif
...
static void check_tty(mparm_T *parmp) {
  ...
  if (exmode_active) {
    ...
    if (
      ...
      vim_free(tofree);
#endif
      mch_errmsg(_("Vim: Error: This version of Vim does not run in a Cygwin "
                   "terminal\n"));
      exit(1);
    }
#endif
 ...
 * Also does recovery if "recoverymode" set.
 */
static void create_windows(mparm_T *parmp UNUSED) {
  int dorewind;
  int done = 0;
 ...
 */
static void edit_buffers(mparm_T *parmp, char_u *cwd) /* current working dir */
{
 ...
 */
int process_env(char_u *env, int is_viminit) /* when TRUE, called for VIMINIT */
{
  char_u *initstr;
 ...
 */
static void mainerr(int n,       /* one of the ME_ defines */
...
void mainerr_arg_missing(char_u *str) { mainerr(ME_ARG_MISSING, str); }
...
static void usage(void) {
  int i;
  static char *(use[]) = {N_("[file ..]       edit specified file(s)"),
  ...
  main_msg(_("--remote-wait-silent <files>  Same, don't complain if there is "
             "no server"));
  main_msg(_("--remote-tab[-wait][-silent] <files>  As --remote but use tab "
             "page per file"));
  main_msg(_("--remote-send <keys>\tSend <keys> to a Vim server and exit"));
  main_msg(_("--remote-expr <expr>\tEvaluate <expr> in a Vim server and print "
             "result"));
 ...
 */
static int gettimeofday(struct timeval *tv, char *dummy) {
  long t = clock();
 ...
 * have struct timeval.
 */
void time_pop(void *tp) /* actually (struct timeval *) */
{
...
void time_msg(char *mesg, void *tv_start) /* only for do_source: start time;
...
static void set_progpath(char_u *argv0) {
  ...
  if (!mch_isFullName(val)) {
    if (gettail(val) != val && vim_FullName(val, buf, MAXPATHL, TRUE) != FAIL)
      val = buf;
  }
#endif
