    ...
    if (s_dwMax > 1) {
      ...
      while (head != tail) {
        if (s_irCache[head].EventType == WINDOW_BUFFER_SIZE_EVENT &&
            s_irCache[head + 1].EventType == WINDOW_BUFFER_SIZE_EVENT) {
...
static DWORD wait_for_single_object(HANDLE hHandle, DWORD dwMilliseconds) {
 ...
 */
static void unescape_shellxquote(char_u *p, char_u *escaped) {
  int l = (int)STRLEN(p);
  int n;
...
int dyn_libintl_init(void) {
  ...
  dyn_libintl_bind_textdomain_codeset =
      (void *)GetProcAddress(hLibintlDLL, "bind_textdomain_codeset");
...
static char *null_libintl_bindtextdomain(const char *domainname UNUSED,
                                         const char *dirname UNUSED) {
  return NULL;
}

static char *null_libintl_bind_textdomain_codeset(const char *domainname UNUSED,
 ...
 */
static BOOL win32_enable_privilege(LPTSTR lpszPrivilege, BOOL bEnable) {
  BOOL bResult;
  LUID luid;
  HANDLE hToken;
...
static int win32_kbd_patch_key(KEY_EVENT_RECORD *pker) {
 ...
 * Handle FOCUS_EVENT.
 */
static void handle_focus_event(INPUT_RECORD ir) {
 ...
 */
static int WaitForChar(long msec, int ignore_input) {
 ...
 */
static int executable_exists(char *name, char_u **path, int use_path) {
  WCHAR *p;
 ...
 *  TRUE on success
 */
static BOOL ClearConsoleBuffer(WORD wAttribute) {
  CONSOLE_SCREEN_BUFFER_INFO csbi;
  COORD coord;
 ...
 *  TRUE on success
 */
static BOOL FitConsoleWindow(COORD dwBufferSize, BOOL WantAdjust) {
  CONSOLE_SCREEN_BUFFER_INFO csbi;
  COORD dwWindowSize;
 ...
 */
static BOOL SaveConsoleBuffer(ConsoleBuffer *cb) {
  DWORD NumCells;
  COORD BufferCoord;
 ...
 *  TRUE on success
 */
static BOOL RestoreConsoleBuffer(ConsoleBuffer *cb, BOOL RestoreScreen) {
  COORD BufferCoord;
 ...
 * return process ID
 */
long mch_get_pid(void) { return (long)GetCurrentProcessId(); }
...
void mch_set_acl(char_u *fname, vim_acl_T acl) {
  ...
  (void)SetNamedSecurityInfoW(wn,             // Abstract filename
                              SE_FILE_OBJECT, // File Object
                              sec_info,
                              p->pSidOwner, // Ownership information.
                              p->pSidGroup, // Group membership.
 ...
 */
static void ResizeWindow(HANDLE hConsole, SMALL_RECT srWindowRect) {
...
static BOOL vim_create_process(char *cmd, BOOL inherit_handles, DWORD flags,
  ...
  ret = CreateProcessW(NULL,               // Executable name
...
static HINSTANCE vim_shell_execute(char *cmd, INT n_show_cmd) {
  HINSTANCE ret;
  WCHAR *wcmd;
 ...
 */
static int mch_system_classic(char *cmd, int options) {
  STARTUPINFO si;
  ...
  for (;;) {
    ...
    if (l == 0)
      ...
      WriteFile(g_hChildStd_IN_Wr, (char *)lp + written,
                s == NULL ? l : (DWORD)(s - (lp + written)), &len, NULL);
    }
    if (len == (int)l) {
 ...
 */
static int mch_system_piped(char *cmd, int options) {
  STARTUPINFO si;
  PROCESS_INFORMATION pi;
  DWORD ret = 0;
  ...
  vim_create_process(p, TRUE, CREATE_DEFAULT_ERROR_MODE, &si, &pi, NULL, NULL);
      ...
      if (!(options & SHELL_EXPAND) &&
        ...
        if (ta_len > 0 || len > 0) {
          ...
          if (options & SHELL_WRITE)
            ...
            WriteFile(g_hChildStd_IN_Wr, (char *)ta_buf, 1, &len, NULL);
...
static int mch_system_g(char *cmd, int options) {
...
#if !defined(FEAT_GUI_MSWIN) || defined(VIMDLL)
static int mch_system_c(char *cmd, int options) {
  int ret;
  WCHAR *wcmd;
...
static int mch_system(char *cmd, int options) {
#ifdef VIMDLL
  if (gui.in_use || gui.starting)
    ...
    if ((STRNICMP(cmdbase, "start", 5) == 0) && VIM_ISWHITE(cmdbase[5])) {
      ...
      if (flags != CREATE_NEW_CONSOLE) {
        ...
        if (subcmd != NULL) {
          ...
          newcmd = alloc(cmdlen);
          if (newcmd != NULL)
            vim_snprintf((char *)newcmd, cmdlen, "%s /c %s", cmd_shell, subcmd);
          else
            newcmd = cmdbase;
          vim_free(subcmd);
        }
      ...
      cmdlen =
          ...
          ((gui.in_use || gui.starting)
               ? (!s_dont_use_vimrun && p_stmp ? STRLEN(vimrun_path)
                                               : STRLEN(p_sh) + STRLEN(p_shcf))
               : 0) +
#endif
 ...
 */
static BOOL create_pipe_pair(HANDLE handles[2]) {
  static LONG s;
  char name[64];
...
static BOOL terminate_all(HANDLE process, int code) {
  PROCESSENTRY32 pe;
 ...
 */
static void cursor_visible(BOOL fVisible) { s_cursor_visible = fVisible; }
...
static DWORD write_chars(char_u *pchBuf, DWORD cbToWrite) {
  ...
  MultiByteToWideChar(cp, 0, (LPCSTR)pchBuf, cbToWrite, unicodebuf, unibuflen);
...
void mch_write(char_u *s, int len) {
  ...
  while (len--) {
    ...
    if (prefix != 0) {
      ...
      case 'A':
        gotoxy(g_coord.X + 1, max(g_srScrollRegion.Top, g_coord.Y - 1) + 1);
        goto got3;
      ...
      case 'C':
        gotoxy(min(g_srScrollRegion.Right, g_coord.X + 1) + 1, g_coord.Y + 1);
        goto got3;
 ...
 */
void mch_delay(long msec, int ignoreinput UNUSED) {
#if defined(FEAT_GUI_MSWIN) && !defined(VIMDLL)
 ...
 */
char *default_shell(void) { return "cmd.exe"; }
 ...
 */
static void copy_infostreams(char_u *from, char_u *to) {
  WCHAR *fromw;
  WCHAR *tow;
  HANDLE sh;
  WIN32_STREAM_ID sid;
  ...
  if (fromw != NULL && tow != NULL) {
    ...
    sh = CreateFileW(fromw, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
 ...
 */
void set_alist_count(void) { used_alist_count = GARGCOUNT; }
...
void fix_arg_enc(void) {
   ...
   */
  if (global_argc != nArgsW || ArglistW == NULL || used_file_indexes == NULL ||
      used_file_count == 0 || used_alist_count != GARGCOUNT)
    return;
...
static void vtp_exit(void) { reset_console_color_rgb(); }
...
static void vtp_sgr_bulks(int argc, int *args) {
  /* 2('\033[') + 4('255.') * 16 + NUL */
  char_u buf[2 + (4 * 16) + 1];
  char_u *p;
  int i;
...
int has_conpty_working(void) { return conpty_working; }

int is_conpty_stable(void) { return conpty_stable; }
