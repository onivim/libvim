 ...
 */
int find_internal_func(char_u *name) /* name of the function */
{
  int first = 0;
...
static int non_zero_arg(typval_T *argvars) {
  return ((argvars[0].v_type == VAR_NUMBER && argvars[0].vval.v_number != 0) ||
          (argvars[0].v_type == VAR_SPECIAL && argvars[0].vval.v_number == VVAL_TRUE) ||
          (argvars[0].v_type == VAR_STRING && argvars[0].vval.v_string != NULL &&
 ...
 * Returns -1 on error.
 */
static linenr_T tv_get_lnum(typval_T *argvars) {
  typval_T rettv;
  linenr_T lnum;
 ...
 * Returns 0 on error.
 */
static linenr_T tv_get_lnum_buf(typval_T *argvars, buf_T *buf) {
  if (argvars[0].v_type == VAR_STRING && argvars[0].vval.v_string != NULL &&
      argvars[0].vval.v_string[0] == '$' && buf != NULL)
...
static void f_add(typval_T *argvars, typval_T *rettv) {
  ...
  if (argvars[0].v_type == VAR_LIST) {
    if ((l = argvars[0].vval.v_list) != NULL &&
        !var_check_lock(l->lv_lock, (char_u *)N_("add() argument"), TRUE) &&
        list_append_tv(l, &argvars[1]) == OK)
      copy_tv(&argvars[0], rettv);
  } else if (argvars[0].v_type == VAR_BLOB) {
    if ((b = argvars[0].vval.v_blob) != NULL &&
        !var_check_lock(b->bv_lock, (char_u *)N_("add() argument"), TRUE)) {
      int error = FALSE;
 ...
 */
static void set_buffer_lines(buf_T *buf, linenr_T lnum_arg, int append, typval_T *lines,
                             typval_T *rettv) {
  ...
  for (;;) {
    ...
    if (!append && lnum <= curbuf->b_ml.ml_line_count) {
      ...
      if (u_savesub(lnum) == OK &&
          ml_replace_len(lnum, line, (colnr_T)STRLEN(line) + 1, TRUE, TRUE) == OK) {
 ...
 * "argidx()" function
 */
static void f_argidx(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 */
static void f_browse(typval_T *argvars UNUSED, typval_T *rettv) {
#ifdef FEAT_BROWSE
  int save;
  char_u *title;
  char_u *initdir;
  ...
  if (error || title == NULL || initdir == NULL || defname == NULL)
    ...
    rettv->vval.v_string =
        do_browse(save ? BROWSE_SAVE : 0, title, defname, NULL, initdir, NULL, curbuf);
#else
  rettv->vval.v_string = NULL;
#endif
  rettv->v_type = VAR_STRING;
 ...
 */
static void f_browsedir(typval_T *argvars UNUSED, typval_T *rettv) {
#ifdef FEAT_BROWSE
  char_u *title;
  char_u *initdir;
  char_u buf[NUMBUFLEN];
  ...
  if (title == NULL || initdir == NULL)
    ...
    rettv->vval.v_string = do_browse(BROWSE_DIR, title, NULL, NULL, initdir, NULL, curbuf);
#else
  rettv->vval.v_string = NULL;
#endif
  rettv->v_type = VAR_STRING;
...
static void f_bufnr(typval_T *argvars, typval_T *rettv) {
  ...
  if (buf == NULL && argvars[1].v_type != VAR_UNKNOWN &&
      ...
      (name = tv_get_string_chk(&argvars[0])) != NULL && !error)
 ...
 */
static void f_bufwinnr(typval_T *argvars, typval_T *rettv) { buf_win_common(argvars, rettv, TRUE); }

/*
 * "byte2line(byte)" function
 */
static void f_byte2line(typval_T *argvars UNUSED, typval_T *rettv) {
#ifndef FEAT_BYTEOFF
  rettv->vval.v_number = -1;
#else
  long boff = 0;
 ...
 * "byteidx()" function
 */
static void f_byteidx(typval_T *argvars, typval_T *rettv) { byteidx(argvars, rettv, FALSE); }

/*
 * "byteidxcomp()" function
 */
static void f_byteidxcomp(typval_T *argvars, typval_T *rettv) { byteidx(argvars, rettv, TRUE); }
 ...
 * "ch_close()" function
 */
static void f_ch_close(typval_T *argvars, typval_T *rettv UNUSED) {
 ...
 * "ch_close()" function
 */
static void f_ch_close_in(typval_T *argvars, typval_T *rettv UNUSED) {
  channel_T *channel = get_channel_arg(&argvars[0], TRUE, FALSE, 0);
 ...
 * "ch_info()" function
 */
static void f_ch_info(typval_T *argvars, typval_T *rettv UNUSED) {
  channel_T *channel = get_channel_arg(&argvars[0], FALSE, FALSE, 0);
 ...
 * "ch_log()" function
 */
static void f_ch_log(typval_T *argvars, typval_T *rettv UNUSED) {
 ...
 * "ch_logfile()" function
 */
static void f_ch_logfile(typval_T *argvars, typval_T *rettv UNUSED) {
  char_u *fname;
  char_u *opt = (char_u *)"";
  char_u buf[NUMBUFLEN];
 ...
 */
static void f_ch_setoptions(typval_T *argvars, typval_T *rettv UNUSED) {
  channel_T *channel;
  jobopt_T opt;
 ...
 * "changenr()" function
 */
static void f_changenr(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 */
static void f_cindent(typval_T *argvars UNUSED, typval_T *rettv) { rettv->vval.v_number = -1; }
 ...
 */
static void f_clearmatches(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
 ...
 */
static void f_confirm(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
 ...
 * "copy()" function
 */
static void f_copy(typval_T *argvars, typval_T *rettv) { item_copy(&argvars[0], rettv, FALSE, 0); }
 ...
 */
static void f_cscope_connection(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {}
 ...
 */
static void f_did_filetype(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
 ...
 */
static void f_diff_filler(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_DIFF
 ...
 * "diff_hlID()" function
 */
static void f_diff_hlID(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_DIFF
 ...
 * "environ()" function
 */
static void f_environ(typval_T *argvars UNUSED, typval_T *rettv) {
  int i = 0;
  char_u *entry, *value;
#ifdef MSWIN
...
static void f_escape(typval_T *argvars, typval_T *rettv) {
  ...
  rettv->vval.v_string =
      vim_strsave_escaped(tv_get_string(&argvars[0]), tv_get_string_buf(&argvars[1], buf));
  rettv->v_type = VAR_STRING;
}
 ...
 */
static void f_eventhandler(typval_T *argvars UNUSED, typval_T *rettv) {
...
static void f_executable(typval_T *argvars, typval_T *rettv) {
  ...
  rettv->vval.v_number =
      mch_can_exe(name, NULL, TRUE) || (gettail(name) != name && mch_can_exe(name, NULL, FALSE));
}
...
static char_u *get_list_line(int c UNUSED, void *cookie, int indent UNUSED) {
 ...
 * "execute()" function
 */
static void f_execute(typval_T *argvars, typval_T *rettv) { execute_common(argvars, rettv, 0); }
 ...
 * "feedkeys()" function
 */
static void f_feedkeys(typval_T *argvars, typval_T *rettv UNUSED) {
  int remap = TRUE;
  int insert = FALSE;
  char_u *keys, *flags;
  char_u nbuf[NUMBUFLEN];
  ...
  if (*fname != NUL && !error) {
    do {
      ...
      fresult = find_file_in_path_option(
          first ? fname : NULL, first ? (int)STRLEN(fname) : 0, 0, first, path, find_what,
          curbuf->b_ffname, find_what == FINDFILE_DIR ? (char_u *)"" : curbuf->b_p_sua);
      first = FALSE;
 ...
 * "filter()" function
 */
static void f_filter(typval_T *argvars, typval_T *rettv) { filter_map(argvars, rettv, FALSE); }
 ...
 * "foldclosed()" function
 */
static void foldclosed_both(typval_T *argvars UNUSED, typval_T *rettv, int end UNUSED) {
#ifdef FEAT_FOLDING
  linenr_T lnum;
  linenr_T first, last;
 ...
 * "foldlevel()" function
 */
static void f_foldlevel(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_FOLDING
  linenr_T lnum;
 ...
 * "foldtext()" function
 */
static void f_foldtext(typval_T *argvars UNUSED, typval_T *rettv) {
#ifdef FEAT_FOLDING
  linenr_T foldstart;
  linenr_T foldend;
  char_u *dashes;
 ...
 */
static void f_foldtextresult(typval_T *argvars UNUSED, typval_T *rettv) {
#ifdef FEAT_FOLDING
  linenr_T lnum;
  char_u *text;
 ...
 * "foreground()" function
 */
static void f_foreground(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_GUI
  if (gui.in_use) {
...
static void common_function(typval_T *argvars, typval_T *rettv, int is_funcref) {
  ...
  /* Don't check an autoload name for existence here. */
           ...
           (is_funcref ? find_func(trans_name) == NULL : !translated_function_exists(trans_name)))
 ...
 * "funcref()" function
 */
static void f_funcref(typval_T *argvars, typval_T *rettv) { common_function(argvars, rettv, TRUE); }
 ...
 */
static void f_garbagecollect(typval_T *argvars, typval_T *rettv UNUSED) {
...
static dict_T *get_buffer_info(buf_T *buf) {
  ...
  dict_add_number(dict, "hidden", buf->b_ml.ml_mfp != NULL && buf->b_nwindows == 0);
...
static void f_getbufinfo(typval_T *argvars, typval_T *rettv) {
  ...
  FOR_ALL_BUFFERS(buf) {
    ...
    if (filtered && ((sel_bufloaded && buf->b_ml.ml_mfp == NULL) ||
                     (sel_buflisted && !buf->b_p_bl) || (sel_bufmodified && !buf->b_changed)))
      continue;
 ...
 */
static void get_buffer_lines(buf_T *buf, linenr_T start, linenr_T end, int retlist,
                             typval_T *rettv) {
  char_u *p;
...
static void f_getchangelist(typval_T *argvars, typval_T *rettv) {
  ...
  list_append_number(
      rettv->vval.v_list,
      (varnumber_T)((buf == curwin->w_buffer) ? curwin->w_changelistidx : buf->b_changelistlen));
 ...
 * "getcharmod()" function
 */
static void f_getcharmod(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 */
static void f_getcharsearch(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 * "getcmdline()" function
 */
static void f_getcmdline(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_STRING;
 ...
 * "getcmdpos()" function
 */
static void f_getcmdpos(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 * "getcmdtype()" function
 */
static void f_getcmdtype(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_STRING;
 ...
 */
static void f_getcmdwintype(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_STRING;
  rettv->vval.v_string = NULL;
}
 ...
 */
static void f_getfontname(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_STRING;
  rettv->vval.v_string = NULL;
#ifdef FEAT_GUI
  if (gui.in_use) {
 ...
 * "getmatches()" function
 */
static void f_getmatches(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_SEARCH_EXTRA
  dict_T *dict;
  matchitem_T *cur;
  int i;
 ...
 * "getpid()" function
 */
static void f_getpid(typval_T *argvars UNUSED, typval_T *rettv) {
...
static void getpos_both(typval_T *argvars, typval_T *rettv, int getcurpos) {
  ...
  if (rettv_list_alloc(rettv) == OK) {
    ...
    if (getcurpos) {
      ...
      update_curswant();
      list_append_number(l, curwin->w_curswant == MAXCOL ? (varnumber_T)MAXCOL
 ...
 * "getcurpos()" function
 */
static void f_getcurpos(typval_T *argvars, typval_T *rettv) { getpos_both(argvars, rettv, TRUE); }

/*
 * "getpos(string)" function
 */
static void f_getpos(typval_T *argvars, typval_T *rettv) { getpos_both(argvars, rettv, FALSE); }

/*
 * "getqflist()" function
 */
static void f_getqflist(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_QUICKFIX
...
static void f_getreg(typval_T *argvars, typval_T *rettv) {
  ...
  if (return_list) {
    rettv->v_type = VAR_LIST;
    rettv->vval.v_list =
        (list_T *)get_reg_contents(regname, (arg2 ? GREG_EXPR_SRC : 0) | GREG_LIST);
...
static void f_gettabvar(typval_T *argvars, typval_T *rettv) {
  ...
  if (tp != NULL && varname != NULL) {
    ...
    if (switch_win(&oldcurwin, &oldtabpage,
                   tp == curtab || tp->tp_firstwin == NULL ? firstwin : tp->tp_firstwin, tp,
                   TRUE) == OK) {
      /* look up the variable */
 ...
 */
static void f_gettabwinvar(typval_T *argvars, typval_T *rettv) { getwinvar(argvars, rettv, 1); }
...
static dict_T *get_win_info(win_T *wp, short tpnr, short winnr) {
  ...
  dict_add_number(dict, "loclist", (bt_quickfix(wp->w_buffer) && wp->w_llist_ref != NULL));
#endif
 ...
 */
static void f_getwinpos(typval_T *argvars UNUSED, typval_T *rettv) {
  int x = -1;
  int y = -1;
 ...
 * "getwinposx()" function
 */
static void f_getwinposx(typval_T *argvars UNUSED, typval_T *rettv) { rettv->vval.v_number = -1; }

/*
 * "getwinposy()" function
 */
static void f_getwinposy(typval_T *argvars UNUSED, typval_T *rettv) { rettv->vval.v_number = -1; }

/*
 * "getwinvar()" function
 */
static void f_getwinvar(typval_T *argvars, typval_T *rettv) { getwinvar(argvars, rettv, 0); }
...
static void f_has(typval_T *argvars, typval_T *rettv) {
  ...
  static char *(has_list[]) = {
    ...
    (defined(USE_FILE_CHOOSER) || defined(FEAT_GUI_MSWIN) || defined(FEAT_GUI_MOTIF))
    "browsefilter",
#endif
#ifdef FEAT_BYTEOFF
    "byte_offset",
  ...
  if (n == FALSE) {
    if (STRNICMP(name, "patch", 5) == 0) {
      if (name[5] == '-' && STRLEN(name) >= 11 && vim_isdigit(name[6]) && vim_isdigit(name[8]) &&
          vim_isdigit(name[10])) {
        ...
        n = (major < VIM_VERSION_MAJOR ||
             (major == VIM_VERSION_MAJOR &&
              (minor < VIM_VERSION_MINOR ||
               (minor == VIM_VERSION_MINOR && has_patch(atoi((char *)name + 10))))));
      } else
...
static void f_has_key(typval_T *argvars, typval_T *rettv) {
  ...
  rettv->vval.v_number = dict_find(argvars[0].vval.v_dict, tv_get_string(&argvars[1]), -1) != NULL;
}
 ...
 * "histadd()" function
 */
static void f_histadd(typval_T *argvars UNUSED, typval_T *rettv) {
#ifdef FEAT_CMDHIST
  int histype;
  char_u *str;
  char_u buf[NUMBUFLEN];
 ...
 * "histdel()" function
 */
static void f_histdel(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_CMDHIST
  int n;
  char_u buf[NUMBUFLEN];
  char_u *str;
  ...
  if (str == NULL)
    ...
    n = del_history_entry(get_histtype(str), tv_get_string_buf(&argvars[1], buf));
  rettv->vval.v_number = n;
#endif
}
 ...
 * "histget()" function
 */
static void f_histget(typval_T *argvars UNUSED, typval_T *rettv) {
#ifdef FEAT_CMDHIST
  int type;
  int idx;
  char_u *str;
 ...
 * "histnr()" function
 */
static void f_histnr(typval_T *argvars UNUSED, typval_T *rettv) {
  int i;
 ...
 * "hostname()" function
 */
static void f_hostname(typval_T *argvars UNUSED, typval_T *rettv) {
  char_u hostname[256];
 ...
 * iconv() function
 */
static void f_iconv(typval_T *argvars UNUSED, typval_T *rettv) {
  char_u buf1[NUMBUFLEN];
  char_u buf2[NUMBUFLEN];
  char_u *from, *to, *str;
  vimconv_T vimconv;
...
static void f_inputdialog(typval_T *argvars, typval_T *rettv) {
  ...
  if (gui.in_use && vim_strchr(p_go, GO_CONDIALOG) == NULL) {
    ...
    if (message != NULL && defstr != NULL &&
        do_dialog(VIM_QUESTION, NULL, message, (char_u *)_("&OK\n&Cancel"), 1, IObuff, FALSE) == 1)
      rettv->vval.v_string = vim_strsave(IObuff);
    else {
      if (message != NULL && defstr != NULL && argvars[1].v_type != VAR_UNKNOWN &&
 ...
 */
static void f_inputrestore(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 * "inputsave()" function
 */
static void f_inputsave(typval_T *argvars UNUSED, typval_T *rettv) {
  ...
  if (argvars[0].v_type == VAR_BLOB) {
    ...
    *(p + before) = val;
    ...
    semsg(_(e_listblobarg), "insert()");
           ...
           !var_check_lock(l->lv_lock, (char_u *)N_("insert() argument"), TRUE)) {
    if (argvars[2].v_type != VAR_UNKNOWN)
      before = (long)tv_get_number_chk(&argvars[2], &error);
    if (error)
      return; /* type error; errmsg already given */
 ...
 * "items(dict)" function
 */
static void f_items(typval_T *argvars, typval_T *rettv) { dict_list(argvars, rettv, 2); }
 ...
 */
static void f_job_setoptions(typval_T *argvars, typval_T *rettv UNUSED) {
  job_T *job = get_job_arg(&argvars[0]);
  jobopt_T opt;
 ...
 * "keys()" function
 */
static void f_keys(typval_T *argvars, typval_T *rettv) { dict_list(argvars, rettv, 0); }

/*
 * "last_buffer_nr()" function.
 */
static void f_last_buffer_nr(typval_T *argvars UNUSED, typval_T *rettv) {
  int n = 0;
  buf_T *buf;
...
static void libcall_common(typval_T *argvars UNUSED, typval_T *rettv, int type) {
#ifdef FEAT_LIBCALL
  char_u *string_in;
  ...
  if (argvars[0].v_type == VAR_STRING && argvars[1].v_type == VAR_STRING) {
    ...
    if (mch_libcall(argvars[0].vval.v_string, argvars[1].vval.v_string, string_in,
                    argvars[2].vval.v_number, string_result, &nr_result) == OK &&
        type == VAR_NUMBER)
 ...
 */
static void f_line2byte(typval_T *argvars UNUSED, typval_T *rettv) {
#ifndef FEAT_BYTEOFF
  rettv->vval.v_number = -1;
#else
  linenr_T lnum;
 ...
 */
static void f_lispindent(typval_T *argvars UNUSED, typval_T *rettv) { rettv->vval.v_number = -1; }
 ...
 * "localtime()" function
 */
static void f_localtime(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 * "map()" function
 */
static void f_map(typval_T *argvars, typval_T *rettv) { filter_map(argvars, rettv, TRUE); }

/*
 * "maparg()" function
 */
static void f_maparg(typval_T *argvars, typval_T *rettv) { get_maparg(argvars, rettv, TRUE); }

/*
 * "mapcheck()" function
 */
static void f_mapcheck(typval_T *argvars, typval_T *rettv) { get_maparg(argvars, rettv, FALSE); }
...
static void find_some_match(typval_T *argvars, typval_T *rettv, matchtype_T type) {
  ...
  if (type == MATCH_LIST || type == MATCH_POS) {
    ...
    if (rettv_list_alloc(rettv) == FAIL)
      goto theend;
    if (type == MATCH_POS && (list_append_string(rettv->vval.v_list, (char_u *)"", 0) == FAIL ||
                              list_append_number(rettv->vval.v_list, (varnumber_T)-1) == FAIL ||
                              list_append_number(rettv->vval.v_list, (varnumber_T)-1) == FAIL ||
                              list_append_number(rettv->vval.v_list, (varnumber_T)-1) == FAIL)) {
    ...
    if (match) {
      if (type == MATCH_POS) {
        ...
        li1->li_tv.vval.v_string =
            vim_strnsave(regmatch.startp[0], (int)(regmatch.endp[0] - regmatch.startp[0]));
...
#ifdef FEAT_SEARCH_EXTRA
static int matchadd_dict_arg(typval_T *tv, char_u **conceal_char, win_T **win) {
  dictitem_T *di;
  ...
  if (dict_find(tv->vval.v_dict, (char_u *)"conceal", -1) != NULL)
    *conceal_char = dict_get_string(tv->vval.v_dict, (char_u *)"conceal", FALSE);
 ...
 * "matchadd()" function
 */
static void f_matchadd(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_SEARCH_EXTRA
  char_u buf[NUMBUFLEN];
 ...
 */
static void f_matchaddpos(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_SEARCH_EXTRA
  char_u buf[NUMBUFLEN];
  char_u *group;
  int prio = 10;
 ...
 * "matcharg()" function
 */
static void f_matcharg(typval_T *argvars UNUSED, typval_T *rettv) {
  if (rettv_list_alloc(rettv) == OK) {
#ifdef FEAT_SEARCH_EXTRA
 ...
 */
static void f_matchdelete(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_SEARCH_EXTRA
  ...
  if (win == NULL)
    ...
    rettv->vval.v_number = match_delete(win, (int)tv_get_number(&argvars[0]), TRUE);
#endif
}
 ...
 * "max()" function
 */
static void f_max(typval_T *argvars, typval_T *rettv) { max_min(argvars, rettv, TRUE); }

/*
 * "min()" function
 */
static void f_min(typval_T *argvars, typval_T *rettv) { max_min(argvars, rettv, FALSE); }
 ...
 */
static void f_prompt_setcallback(typval_T *argvars, typval_T *rettv UNUSED) {
  buf_T *buf;
  callback_T callback;
 ...
 */
static void f_prompt_setinterrupt(typval_T *argvars, typval_T *rettv UNUSED) {
  buf_T *buf;
  callback_T callback;
 ...
 */
static void f_prompt_setprompt(typval_T *argvars, typval_T *rettv UNUSED) {
  buf_T *buf;
  char_u *text;
 ...
 * "pumvisible()" function
 */
static void f_pumvisible(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {}
 ...
 */
static int readdir_checkitem(void *context, char_u *name) {
...
static void f_readfile(typval_T *argvars, typval_T *rettv) {
  ...
  while (cnt < maxline || maxline < 0) {
     ...
     *   newline.  */
    for (p = buf, start = buf; p < buf + readlen || (readlen <= 0 && (prevlen > 0 || binary));
         ++p) {
      if (*p == '\n' || readlen <= 0) {
        listitem_T *li;
        char_u *s = NULL;
 ...
 */
static void f_reg_executing(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 */
static void f_reg_recording(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 * "reltime()" function
 */
static void f_reltime(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_RELTIME
  proftime_T res;
  proftime_T start;
 ...
 */
static void f_reltimefloat(typval_T *argvars UNUSED, typval_T *rettv) {
#ifdef FEAT_RELTIME
  proftime_T tm;
#endif
 ...
 * "reltimestr()" function
 */
static void f_reltimestr(typval_T *argvars UNUSED, typval_T *rettv) {
#ifdef FEAT_RELTIME
  proftime_T tm;
#endif
 ...
 */
static void f_remote_expr(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_STRING;
  rettv->vval.v_string = NULL;
}
 ...
 */
static void f_remote_foreground(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
  // Libvim: noop
}

static void f_remote_peek(typval_T *argvars UNUSED, typval_T *rettv) { rettv->vval.v_number = -1; }

static void f_remote_read(typval_T *argvars UNUSED, typval_T *rettv) {
  char_u *r = NULL;
  rettv->v_type = VAR_STRING;
  rettv->vval.v_string = r;
}
 ...
 */
static void f_remote_send(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_STRING;
  rettv->vval.v_string = NULL;
}
 ...
 */
static void f_remote_startserver(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
  ...
  if (argvars[0].v_type == VAR_DICT) {
    ...
    if (!error) {
      ...
      if (argvars[2].v_type == VAR_UNKNOWN) {
        ...
        p = (char_u *)b->bv_ga.ga_data;
        mch_memmove((char_u *)blob->bv_ga.ga_data, p + idx, (size_t)(end - idx + 1));
        ++blob->bv_refcount;
...
static void f_rename(typval_T *argvars, typval_T *rettv) {
  ...
  if (check_restricted() || check_secure())
    ...
    rettv->vval.v_number =
        vim_rename(tv_get_string(&argvars[0]), tv_get_string_buf(&argvars[1], buf));
}
...
static void f_reverse(typval_T *argvars, typval_T *rettv) {
  ...
  if (argvars[0].v_type != VAR_LIST)
    semsg(_(e_listblobarg), "reverse()");
           ...
           !var_check_lock(l->lv_lock, (char_u *)N_("reverse() argument"), TRUE)) {
    li = l->lv_last;
...
static int search_cmn(typval_T *argvars, pos_T *match_pos, int *flagsp) {
  ...
  subpatnum = searchit(curwin, curbuf, &pos, NULL, dir, pat, 1L, options, RE_SEARCH,
                       (linenr_T)lnum_stop, &tm, NULL);
  if (subpatnum != FAIL) {
    if (flags & SP_SUBPAT)
      retval = subpatnum;
    else
 ...
 */
float_T vim_round(float_T f) { return f > 0 ? floor(f + 0.5) : ceil(f - 0.5); }
...
static void f_screenchars(typval_T *argvars, typval_T *rettv) {
  ...
  if (enc_utf8)
    ...
    for (i = 0; i < Screen_mco && ScreenLinesC[i][off] != 0; ++i)
      list_append_number(rettv->vval.v_list, (varnumber_T)ScreenLinesC[i][off]);
}
 ...
 */
static void f_screencol(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 * "screenrow()" function
 */
static void f_screenrow(typval_T *argvars UNUSED, typval_T *rettv) {
                   ...
                   long time_limit UNUSED)               /* stop after this many msec */
{
  ...
  pat = pat3;
  for (;;) {
    n = searchit(curwin, curbuf, &pos, NULL, dir, pat, 1L, options, RE_SEARCH, lnum_stop, &tm,
                 NULL);
...
static void f_server2client(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->vval.v_number = -1;
}

static void f_serverlist(typval_T *argvars UNUSED, typval_T *rettv) {
  char_u *r = NULL;
 ...
 * "setbufvar()" function
 */
static void f_setbufvar(typval_T *argvars, typval_T *rettv UNUSED) {
  buf_T *buf;
...
static void f_setcharsearch(typval_T *argvars, typval_T *rettv UNUSED) {
  dict_T *d;
  dictitem_T *di;
  char_u *csearch;
  ...
  if ((d = argvars[0].vval.v_dict) != NULL) {
    ...
    if (di != NULL)
      set_csearch_direction((int)tv_get_number(&di->di_tv) ? FORWARD : BACKWARD);
 ...
 * "setenv()" function
 */
static void f_setenv(typval_T *argvars, typval_T *rettv UNUSED) {
 ...
 */
static void set_qf_ll_list(win_T *wp UNUSED, typval_T *list_arg UNUSED, typval_T *action_arg UNUSED,
                           typval_T *what_arg UNUSED, typval_T *rettv) {
#ifdef FEAT_QUICKFIX
  ...
  if (list_arg->v_type != VAR_LIST)
    ...
    if (l != NULL && action && valid_dict &&
        set_errorlist(wp, l, action, (char_u *)(wp == NULL ? ":setqflist()" : ":setloclist()"),
                      d) == OK)
      rettv->vval.v_number = 0;
    --recursive;
  }
 ...
 * "setmatches()" function
 */
static void f_setmatches(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_SEARCH_EXTRA
  list_T *l;
  listitem_T *li;
  dict_T *d;
  ...
  if ((l = argvars[0].vval.v_list) != NULL) {
    ...
    while (li != NULL) {
      ...
      conceal = dict_find(d, (char_u *)"conceal", -1) != NULL
                    ...
                    : NULL;
      if (i == 0) {
        match_add(win, group, dict_get_string(d, (char_u *)"pattern", FALSE), priority, id, NULL,
                  conceal);
      } else {
 ...
 */
static void f_settabwinvar(typval_T *argvars, typval_T *rettv) { setwinvar(argvars, rettv, 1); }
 ...
 * "setwinvar()" function
 */
static void f_setwinvar(typval_T *argvars, typval_T *rettv) { setwinvar(argvars, rettv, 0); }
 ...
 * shiftwidth() function
 */
static void f_shiftwidth(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->vval.v_number = 0;
...
static void f_sign_getplaced(typval_T *argvars, typval_T *rettv) {
  ...
  if (argvars[0].v_type != VAR_UNKNOWN) {
    ...
    if (argvars[1].v_type != VAR_UNKNOWN) {
      if (argvars[1].v_type != VAR_DICT || ((dict = argvars[1].vval.v_dict) == NULL)) {
        emsg(_(e_dictreq));
        return;
      }
...
static void f_sign_place(typval_T *argvars, typval_T *rettv) {
  ...
  if (argvars[4].v_type != VAR_UNKNOWN) {
    if (argvars[4].v_type != VAR_DICT || ((dict = argvars[4].vval.v_dict) == NULL)) {
      emsg(_(e_dictreq));
      goto cleanup;
    }
...
static int item_compare2(const void *s1, const void *s2) {
  ...
  res = call_func(func_name, -1, &rettv, 2, argv, NULL, 0L, 0L, &dummy, TRUE, partial,
...
static void do_sort_uniq(typval_T *argvars, typval_T *rettv, int sort) {
  ...
  if (argvars[0].v_type != VAR_LIST)
    ...
    l = argvars[0].vval.v_list;
    if (l == NULL ||
        var_check_lock(l->lv_lock, (char_u *)(sort ? N_("sort() argument") : N_("uniq() argument")),
                       TRUE))
      goto theend;
    rettv_list_set(rettv, l);
    ...
    if (sort) {
      ...
      if ((info.item_compare_func != NULL || info.item_compare_partial != NULL) &&
        ...
        qsort((void *)ptrs, (size_t)len, sizeof(sortItem_T),
              info.item_compare_func == NULL && info.item_compare_partial == NULL ? item_compare
                                                                                  : item_compare2);
 ...
 * "sort({list})" function
 */
static void f_sort(typval_T *argvars, typval_T *rettv) { do_sort_uniq(argvars, rettv, TRUE); }

/*
 * "uniq({list})" function
 */
static void f_uniq(typval_T *argvars, typval_T *rettv) { do_sort_uniq(argvars, rettv, FALSE); }
 ...
 */
static void f_spellbadword(typval_T *argvars UNUSED, typval_T *rettv) {
  char_u *word = (char_u *)"";
  ...
  list_append_string(
      ...
      (char_u *)(attr == HLF_SPB
                     ? "bad"
                     : attr == HLF_SPR ? "rare"
                                       : attr == HLF_SPL ? "local" : attr == HLF_SPC ? "caps" : ""),
      -1);
}
 ...
 */
static void f_spellsuggest(typval_T *argvars UNUSED, typval_T *rettv) {
  ...
  if (regmatch.regprog != NULL) {
    ...
    while (*str != NUL || keepempty) {
      ...
      if (keepempty || end > str ||
          (rettv->vval.v_list->lv_len > 0 && *str != NUL && match && end < regmatch.endp[0])) {
 ...
 */
static void f_synID(typval_T *argvars UNUSED, typval_T *rettv) {
  int id = 0;
 ...
 */
static void f_synIDattr(typval_T *argvars UNUSED, typval_T *rettv) {
  char_u *p = NULL;
  rettv->v_type = VAR_STRING;
  rettv->vval.v_string = p;
}
 ...
 */
static void f_synIDtrans(typval_T *argvars UNUSED, typval_T *rettv) {
  int id;
 ...
 */
static void f_synconcealed(typval_T *argvars UNUSED, typval_T *rettv) {
 ...
 */
static void f_synstack(typval_T *argvars UNUSED, typval_T *rettv) { rettv_list_set(rettv, NULL); }
 ...
 */
static void f_tabpagebuflist(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
  tabpage_T *tp;
  win_T *wp = NULL;
 ...
 * "tabpagenr()" function
 */
static void f_tabpagenr(typval_T *argvars UNUSED, typval_T *rettv) {
  int nr = 1;
  char_u *arg;
 ...
 */
static void f_tabpagewinnr(typval_T *argvars UNUSED, typval_T *rettv) {
  int nr = 1;
  tabpage_T *tp;
 ...
 * "tagfiles()" function
 */
static void f_tagfiles(typval_T *argvars UNUSED, typval_T *rettv) {
  char_u *fname;
  tagname_T tn;
  int first;
 ...
 * "tempname()" function
 */
static void f_tempname(typval_T *argvars UNUSED, typval_T *rettv) {
  static int x = 'A';
 ...
 */
static void f_test_alloc_fail(typval_T *argvars, typval_T *rettv UNUSED) {
  if (argvars[0].v_type != VAR_NUMBER || argvars[0].vval.v_number <= 0 ||
      argvars[1].v_type != VAR_NUMBER || argvars[1].vval.v_number < 0 ||
 ...
 * "test_autochdir()"
 */
static void f_test_autochdir(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
 ...
 * "test_feedinput()"
 */
static void f_test_feedinput(typval_T *argvars, typval_T *rettv UNUSED) {
#ifdef USE_INPUT_BUF
 ...
 */
static void f_test_option_not_set(typval_T *argvars, typval_T *rettv UNUSED) {
  char_u *name = (char_u *)"";
 ...
 */
static void f_test_override(typval_T *argvars, typval_T *rettv UNUSED) {
  char_u *name = (char_u *)"";
  int val;
 ...
 */
static void f_test_garbagecollect_now(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
 ...
 */
static void f_test_ignore_error(typval_T *argvars, typval_T *rettv UNUSED) {
  ignore_error_for_testing(tv_get_string(&argvars[0]));
}

static void f_test_null_blob(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_BLOB;
  rettv->vval.v_blob = NULL;
}

#ifdef FEAT_JOB_CHANNEL
static void f_test_null_channel(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_CHANNEL;
  rettv->vval.v_channel = NULL;
}
#endif

static void f_test_null_dict(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv_dict_set(rettv, NULL);
}

#ifdef FEAT_JOB_CHANNEL
static void f_test_null_job(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_JOB;
  rettv->vval.v_job = NULL;
}
#endif

static void f_test_null_list(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv_list_set(rettv, NULL);
}

static void f_test_null_partial(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_PARTIAL;
  rettv->vval.v_partial = NULL;
}

static void f_test_null_string(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_STRING;
  rettv->vval.v_string = NULL;
}

#ifdef FEAT_GUI
static void f_test_scrollbar(typval_T *argvars, typval_T *rettv UNUSED) {
  char_u *which;
  long value;
  int dragging;
  scrollbar_T *sb = NULL;
...
static void f_test_settime(typval_T *argvars, typval_T *rettv UNUSED) {
 ...
 */
static void f_timer_pause(typval_T *argvars, typval_T *rettv UNUSED) {
  timer_T *timer = NULL;
 ...
 */
static void f_timer_stop(typval_T *argvars, typval_T *rettv UNUSED) {
  timer_T *timer;
 ...
 */
static void f_timer_stopall(typval_T *argvars UNUSED, typval_T *rettv UNUSED) { stop_all_timers(); }
#endif
 ...
 */
static void f_undofile(typval_T *argvars UNUSED, typval_T *rettv) {
  rettv->v_type = VAR_STRING;
#ifdef FEAT_PERSISTENT_UNDO
  {
 ...
 * "undotree()" function
 */
static void f_undotree(typval_T *argvars UNUSED, typval_T *rettv) {
  if (rettv_dict_alloc(rettv) == OK) {
    dict_T *dict = rettv->vval.v_dict;
    list_T *list;
 ...
 */
static void f_values(typval_T *argvars, typval_T *rettv) { dict_list(argvars, rettv, 1); }
 ...
 */
static void f_wildmenumode(typval_T *argvars UNUSED, typval_T *rettv UNUSED) {
#ifdef FEAT_WILDMENU
 ...
 * "wincol()" function
 */
static void f_wincol(typval_T *argvars UNUSED, typval_T *rettv) {
  validate_cursor();
 ...
 * "winline()" function
 */
static void f_winline(typval_T *argvars UNUSED, typval_T *rettv) {
  validate_cursor();
 ...
 * "winnr()" function
 */
static void f_winnr(typval_T *argvars UNUSED, typval_T *rettv) {
  int nr = 1;
 ...
 * "winrestcmd()" function
 */
static void f_winrestcmd(typval_T *argvars UNUSED, typval_T *rettv) {
  win_T *wp;
  int winnr = 1;
  garray_T ga;
  char_u buf[50];
 ...
 */
static void f_winrestview(typval_T *argvars, typval_T *rettv UNUSED) {
  dict_T *dict;
 ...
 */
static void f_winsaveview(typval_T *argvars UNUSED, typval_T *rettv) {
  dict_T *dict;
 ...
 * "wordcount()" function
 */
static void f_wordcount(typval_T *argvars UNUSED, typval_T *rettv) {
  if (rettv_dict_alloc(rettv) == FAIL)
    return;
