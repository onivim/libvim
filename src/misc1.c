 ...
 */
int set_indent(int size, /* measured in spaces */
               int flags) {
  char_u *p;
  char_u *newline;
  char_u *oldline;
 ...
 */
int get_breakindent_win(win_T *wp, char_u *line) /* start of the line */
{
 ...
 */
void vim_beep(unsigned val) /* one of the BO_ values, e.g., BO_OPER */
{
#ifdef FEAT_EVAL
...
void free_homedir(void) { vim_free(homedir); }

#ifdef FEAT_CMDL_COMPL
void free_users(void) { ga_clear_strings(&ga_users); }
#endif
#endif
 ...
 * again soon.
 */
char_u *expand_env_save(char_u *src) { return expand_env_save_opt(src, FALSE); }
 ...
 */
void expand_env(char_u *src, /* input string e.g. "$HOME/vim.hlp" */
...
void expand_env_esc(char_u *srcp,     /* input string e.g. "$HOME/vim.hlp" */
                    char_u *dst,      /* where to put the result */
                    int dstlen,       /* maximum length of the result */
                    ...
                    char_u *startstr) /* start again after this (can be NULL) */
{
  ...
  while (*src && dstlen > 0) {
    ...
    if ((*src == '$'
      ...
      /* home directory */
      else if (src[1] == NUL || vim_ispathsep(src[1]) ||
               vim_strchr((char_u *)" ,\t\n", src[1]) != NULL) {
        var = homedir;
        tail = src + 1;
      } else /* user directory */
      {
        ...
        *var = NUL;
...
#if defined(HAVE_GETPWNAM) && defined(HAVE_PWD_H)
        {
          ...
          struct passwd *pw = (*dst == NUL) ? NULL : getpwnam((char *)dst + 1);
         ...
         */
        {
          ...
          next_path = paths;
          while (*next_path) {
            for (path = next_path; *next_path && *next_path != ','; next_path++)
              ;
            if (*next_path)
              *next_path++ = NUL;
            STRCPY(test, path);
...
static char_u *remove_tail(char_u *p, char_u *pend, char_u *name) {
  ...
  if (newend >= p && fnamencmp(newend, name, len - 1) == 0 &&
 ...
 */
char_u *get_env_name(expand_T *xp UNUSED, int idx) {
#ifndef __WIN32__
 ...
 */
static void add_user(char_u *user, int need_copy) {
 ...
 */
char_u *get_users(expand_T *xp UNUSED, int idx) {
  init_users();
 ...
 */
void home_replace(buf_T *buf,  /* when not NULL, check for help files */
 ...
 */
char_u *home_replace_save(buf_T *buf,  /* when not NULL, check for help files */
                          char_u *src) /* input file name */
{
  char_u *dst;
  unsigned len;
 ...
 * TRUE.
 */
int fullpathcmp(char_u *s1, char_u *s2,
 ...
 * Returns OK or FAIL.
 */
int expand_wildcards_eval(char_u **pat,   /* pointer to input pattern */
 ...
 */
int expand_wildcards(int num_pat,     /* number of input patterns */
      ...
      *p++ = *path_end++;
      ...
      s = p + 1;
               ...
               (vim_strchr((char_u *)"*?[{~$", *path_end) != NULL ||
                (!p_fic && (flags & EW_ICASE) && isalpha(PTR2CHAR(path_end)))))
      e = p;
    if (has_mbyte) {
      len = (*mb_ptr2len)(path_end);
      STRNCPY(p, path_end, len);
 ...
 */
int gen_expand_wildcards(int num_pat,    /* number of input patterns */
                         char_u **pat,   /* array of input patterns */
                         int *num_file,  /* resulting number of files */
                         char_u ***file, /* array of resulting files */
                         int flags)      /* EW_* flags */
 ...
 */
int goto_im(void) { return (p_im && stuff_empty() && typebuf_typed()); }
 ...
 */
int vim_FullName(char_u *fname, char_u *buf, int len,
