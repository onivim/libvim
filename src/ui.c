...
void clip_update_selection(VimClipboard *clip)
{
  ...
  if (!redo_VIsual_busy && VIsual_active && (State & NORMAL))
  {
    ...
    if (!EQUAL_POS(clip->start, start) || !EQUAL_POS(clip->end, end) || clip->vmode != VIsual_mode)
    {
...
void clip_own_selection(VimClipboard *cbd)
{
  ...
  if (cbd->available)
  {
    ...
    if (!was_owned && (cbd == &clip_star || cbd == &clip_plus))
    {
      ...
      if (cbd->owned && (get_real_state() == VISUAL || get_real_state() == SELECTMODE) && (cbd == &clip_star ? clip_isautosel_star() : clip_isautosel_plus()) && HL_ATTR(HLF_V) != HL_ATTR(HLF_VNC))
...
void clip_lose_selection(VimClipboard *cbd)
{
  ...
  if (visual_selection)
  {
    ...
    if (was_owned && (get_real_state() == VISUAL || get_real_state() == SELECTMODE) && (cbd == &clip_star ? clip_isautosel_star() : clip_isautosel_plus()) && HL_ATTR(HLF_V) != HL_ATTR(HLF_VNC))
    {
    ...
    int_u repeated_click)
{
  ...
  case SELECT_MODE_WORD:
    ...
    if (row == cb->prev.lnum && col >= (int)cb->word_start_col && col < (int)cb->word_end_col && !repeated_click)
      return;
...
void clip_may_redraw_selection(int row, int col, int len)
{
  ...
  if (clip_star.state != SELECT_CLEARED && row >= clip_star.start.lnum && row <= clip_star.end.lnum)
  {
...
void clip_gen_set_selection(VimClipboard *cbd)
{
  if (!clip_did_set_selection)
  {
    ...
    if ((cbd == &clip_plus && (clip_unnamed_saved & CLIP_UNNAMED_PLUS)) || (cbd == &clip_star && (clip_unnamed_saved & CLIP_UNNAMED)))
    {
