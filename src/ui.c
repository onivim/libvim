 ...
 */
void ui_set_shellsize(int mustset UNUSED) /* set by the user */
{
#ifdef FEAT_GUI
  if (gui.in_use)
...
void ui_breakcheck(void) { ui_breakcheck_force(FALSE); }
...
void clip_update_selection(VimClipboard *clip) {
  ...
  if (!redo_VIsual_busy && VIsual_active && (State & NORMAL)) {
    ...
    if (!EQUAL_POS(clip->start, start) || !EQUAL_POS(clip->end, end) ||
 ...
 * right text.
 */
int is_clipboard_needs_update() { return clipboard_needs_update; }
...
void clip_may_redraw_selection(int row, int col, int len) {
  ...
  if (clip_star.state != SELECT_CLEARED && row >= clip_star.start.lnum &&
 ...
 */
void clip_scroll_selection(int rows) /* negative for scroll down */
{
  int lnum;
 ...
 * 0: invert (GUI only).
 */
static void clip_invert_area(int row1, int col1, int row2, int col2, int how) {
  int invert = FALSE;
 ...
 */
void clip_copy_modeless_selection(int both UNUSED) {
  char_u *buffer;
  char_u *bufp;
  int row;
  int start_col;
...
static void clip_get_word_boundaries(VimClipboard *cb, int row, int col) {
  int start_class;
  int temp_col;
  char_u *p;
  int mboff;
 ...
 * line.
 */
static int clip_get_line_end(int row) {
  int i;
...
int clip_gen_owner_exists(VimClipboard *cbd UNUSED) {
#ifdef FEAT_XCLIPBOARD
#ifdef FEAT_GUI_GTK
  if (gui.in_use)
...
int vim_is_input_buf_full(void) { return (inbufcount >= INBUFLEN); }

int vim_is_input_buf_empty(void) { return (inbufcount == 0); }

#if defined(FEAT_OLE) || defined(PROTO)
int vim_free_in_input_buf(void) { return (INBUFLEN - inbufcount); }
#endif

#if defined(FEAT_GUI_GTK) || defined(PROTO)
int vim_used_in_input_buf(void) { return inbufcount; }
#endif
...
void trash_input_buf(void) { inbufcount = 0; }
 ...
 */
static void clip_x11_timestamp_cb(Widget w, XtPointer n UNUSED, XEvent *event,
                                  Boolean *cont UNUSED) {
  Atom actual_type;
  int format;
...
static void clip_x11_request_selection_cb(Widget w UNUSED, XtPointer success, Atom *sel_atom,
  ...
  else if (*type == compound_text_atom || *type == utf8_atom ||
...
static void clip_x11_lose_ownership_cb(Widget w UNUSED, Atom *sel_atom) {
  if (*sel_atom == clip_plus.sel_atom)
 ...
 */
void ui_focus_change(int in_focus) /* TRUE if focus gained. */
{
